// ===============================
// ZENTRYX PRO - PROYECTOS V1011
// V1011 - DETALLE VISIBLE DE REGLAS HÍBRIDAS Y REFERENCIA DE CÁLCULO
// ===============================
(function(){
"use strict";
...
function propuestasEstrategiaHTML(){
  ...
  return PROPUESTAS.map((op,i)=>{
    ...
    return `<div class="zx_pr_strategy_card">
      <div class="zx_pr_gen_top"><div><b>${limpiar(op.nombre)}</b><span>${calc?"Cálculo de referencia · Versión "+limpiar(calc.version):"Sin cálculo de referencia"}</span></div>${puedeEditar()?`<button type="button" data-pr-strategy-open="${limpiar(op.id)}">Estrategia</button>`:""}</div>
