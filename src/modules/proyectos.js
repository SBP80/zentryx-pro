...
function metaSaneamientoProyecto(p){const m=p&&p.inmueble_meta&&typeof p.inmueble_meta==="object"&&!Array.isArray(p.inmueble_meta)?p.inmueble_meta:{},x=m.saneamiento&&typeof m.saneamiento==="object"&&!Array.isArray(m.saneamiento)?m.saneamiento:{};return Object.assign({version:1,uso:"privado",tipo_red:"sin_indicar",puntos:[],notas:""},x,{puntos:Array.isArray(x.puntos)?x.puntos:[]})}
function generarSaneamientoDesdeFontaneria(p,existentes=[]){const font=metaFontaneriaProyecto(p),manuales=(existentes||[]).filter(x=>x.origen!=="fontaneria"),auto=[];font.puntos.forEach(fp=>{const tipo=tipoSaneamientoDesdeFont(fp.tipo);if(!tipo)return;auto.push({punto_id:"san_"+(fp.punto_id||uidFontaneria()),tipo,cantidad:Math.max(1,Math.round(nExt(fp.cantidad,1))),ubicacion:fp.ubicacion||"",ramal:fp.ramal||"",origen:"fontaneria",origen_punto_fontaneria_id:fp.punto_id||null,caudal_ls:null,diametro_manual_mm:null})});return [...auto,...manuales]}
function filaSaneamientoHTML(x={}){const id=x.punto_id||("san_"+uidFontaneria());return `<article class="zx_pr_plumb_point" data-pr-drain-id="${limpiar(id)}" data-pr-drain-origin="${limpiar(x.origen||"manual")}"><div class="zx_pr_plumb_point_head"><b>${x.origen==="fontaneria"?"Desde Fontanería":"Punto añadido"}</b><button type="button" data-pr-drain-del aria-label="Quitar punto">×</button></div><label>Desagüe / aparato<select data-pr-drain-type>${opcionesSaneamiento(x.tipo)}</select></label><div class="zx_pr_grid2"><label>Cantidad <span class="zx_pr_unit">ud</span><input data-pr-drain-qty type="number" min="1" step="1" inputmode="numeric" value="${limpiar(Math.max(1,Math.round(nExt(x.cantidad,1))))}"></label><label>Zona / estancia<input data-pr-drain-room value="${limpiar(x.ubicacion||"")}" placeholder="Baño 1, cocina…"></label><label>Ramal / colector<input data-pr-drain-branch value="${limpiar(x.ramal||"")}" placeholder="Ramal baño, bajante B1…"></label><label>Caudal continuo · opcional <span class="zx_pr_unit">L/s</span><input data-pr-drain-flow type="number" min="0" step="0.001" inputmode="decimal" value="${x.caudal_ls!=null?limpiar(x.caudal_ls):""}" placeholder="Condensados"></label></div><div class="zx_pr_plumb_rates" data-pr-drain-rates></div></article>`}
...
function conectarSaneamientoPunto(row){["change","input"].forEach(ev=>row.addEventListener(ev,()=>{pintarSaneamientoPunto(row);actualizarResumenSaneamiento()}));row.querySelector("[data-pr-drain-del]").onclick=()=>{row.remove();actualizarResumenSaneamiento()};pintarSaneamientoPunto(row)}
function leerSaneamientoFormulario(){return [...document.querySelectorAll("[data-pr-drain-id]")].map(r=>({punto_id:r.dataset.prDrainId||("san_"+uidFontaneria()),tipo:r.querySelector("[data-pr-drain-type]")?.value||"",cantidad:Math.max(1,Math.round(nExt(r.querySelector("[data-pr-drain-qty]")?.value,1))),ubicacion:String(r.querySelector("[data-pr-drain-room]")?.value||"").trim(),ramal:String(r.querySelector("[data-pr-drain-branch]")?.value||"").trim(),caudal_ls:numFontOpt(r.querySelector("[data-pr-drain-flow]")?.value),origen:r.dataset.prDrainOrigin||"manual"})).filter(x=>x.tipo)}
function resumenSaneamiento(puntos,uso){let ud=0,pend=0,uds=0;(puntos||[]).forEach(x=>{const d=defSaneamiento(x.tipo),q=Math.max(1,Math.round(nExt(x.cantidad,1)));uds+=q;if(!d){pend++;return}let val=null;if(d.continuo)val=x.caudal_ls!=null?Math.max(1,Math.ceil(Number(x.caudal_ls)/0.03))*q:null;else val=uso==="publico"?d.ud_pub:d.ud_priv;if(val==null)pend++;else ud+=d.continuo?val:val*q});return {puntos:uds,ud:redondearCalc(ud,1),pendientes:pend}}
function esquemaSaneamientoSVG(x){x=x||{};const pts=Array.isArray(x.puntos)?x.puntos:[],uso=x.uso||"privado",h=Math.max(330,150+Math.max(1,pts.length)*58),x0=670,collector=300,y0=112,step=58;let rows="";pts.forEach((pt,i)=>{const y=y0+i*step,d=defSaneamiento(pt.tipo),rd=d?(uso==="publico"?d.d_pub:d.d_priv):null,diam=rd?`Ø ${rd} mm`:"Ø pendiente";rows+=`<rect x="${x0}" y="${y-19}" width="190" height="42" rx="9" fill="#fff" stroke="#cbd5e1"/><text x="${x0+10}" y="${y-2}" font-size="11" font-weight="700" fill="#0f172a">${limpiar((d?d.nombre:pt.tipo)+(pt.cantidad>1?" ×"+pt.cantidad:"" )).slice(0,32)}</text><text x="${x0+10}" y="${y+14}" font-size="9" fill="#64748b">${limpiar(pt.ubicacion||pt.ramal||"Sin zona").slice(0,32)}</text><line x1="${collector}" y1="${y}" x2="${x0}" y2="${y}" stroke="#7c5a3b" stroke-width="4"/><rect x="${collector+70}" y="${y-13}" width="90" height="22" rx="11" fill="#f3e8dc"/><text x="${collector+81}" y="${y+2}" font-size="9" font-weight="700" fill="#6b4423">${limpiar(diam)}</text>`});return `<div class="zx_pr_scheme_wrap"><svg class="zx_pr_scheme" viewBox="0 0 900 ${h}" role="img" aria-label="Esquema de saneamiento"><rect width="900" height="${h}" rx="18" fill="#fff"/><text x="24" y="32" font-size="16" font-weight="800" fill="#071330">Esquema de saneamiento</text><rect x="24" y="48" width="150" height="24" rx="12" fill="#f3e8dc"/><text x="38" y="64" font-size="10" font-weight="700" fill="#6b4423">Aguas residuales</text><rect x="215" y="88" width="170" height="54" rx="10" fill="#f3e8dc" stroke="#7c5a3b"/><text x="234" y="111" font-size="12" font-weight="700" fill="#4a2f1a">Colector / bajante</text><text x="234" y="130" font-size="10" fill="#6b4423">Dimensionado por UD y pendiente</text>${rows}<line x1="300" y1="142" x2="300" y2="${h-55}" stroke="#7c5a3b" stroke-width="7"/><path d="M 300 ${h-55} H 110" stroke="#7c5a3b" stroke-width="7" fill="none"/><rect x="24" y="${h-86}" width="120" height="46" rx="10" fill="#e2e8f0" stroke="#64748b"/><text x="42" y="${h-60}" font-size="11" font-weight="700" fill="#334155">Salida / arqueta</text><text x="24" y="${h-18}" font-size="10" fill="#64748b">Propuesta inicial. Los colectores, pendientes, ventilación y longitudes se completan en el cálculo detallado.</text></svg></div>`}
function bloqueSaneamientoFichaHTML(p){if(!especialidadesProyecto(p).includes("saneamiento"))return "";const x=metaSaneamientoProyecto(p),font=metaFontaneriaProyecto(p),propuestos=generarSaneamientoDesdeFontaneria(p,x.puntos),pts=x.puntos.length?x.puntos:propuestos,res=resumenSaneamiento(pts,x.uso);return `<section class="zx_pr_section"><div class="zx_pr_section_head"><div><h3>Saneamiento · aguas residuales</h3><span>${pts.length?res.puntos+" punto(s) · "+fmtFontaneria(res.ud,0,1)+" UD":"Sin puntos guardados"}</span></div>${puedeEditar()?`<button id="pr_drain_edit" class="zx_pr_small_primary" type="button">${x.puntos.length?"Editar saneamiento":"Configurar saneamiento"}</button>`:""}</div>${!x.puntos.length&&font.puntos.length?`<div class="zx_pr_norm_box"><b>Propuesta disponible desde Fontanería</b><span>Zentryx puede crear los desagües iniciales de los aparatos que ya tienen punto de suministro. Después podrás añadir sumideros, condensados, descargas de seguridad, vaciados y otros puntos.</span></div>`:""}${pts.length?`<div class="zx_pr_plumb_summary"><div><span>Puntos previstos</span><b>${res.puntos} ud</b></div><div><span>Unidades de desagüe</span><b>${fmtFontaneria(res.ud,0,1)} UD</b></div><div><span>Datos pendientes</span><b>${res.pendientes}</b></div></div>${esquemaSaneamientoSVG(Object.assign({},x,{puntos:pts}))}`:`<div class="zx_pr_empty">Saneamiento está activo, pero todavía no hay puntos propuestos o añadidos.</div>`}</section>`}
function formularioSaneamiento(p){instalarCSSFontSanV4();const x=metaSaneamientoProyecto(p),auto=!x.puntos.length&&metaFontaneriaProyecto(p).puntos.length,pts=auto?generarSaneamientoDesdeFontaneria(p,[]):x.puntos.slice(),m=modal(`<div class="zx_pr_top_actions"><button id="pr_drain_back" type="button">← Volver</button><button id="pr_drain_save" class="primary" type="button">Guardar saneamiento</button></div><div class="zx_pr_form_head"><span>SANEAMIENTO · ASISTENTE</span><h2>${limpiar(p.nombre)}</h2></div><div class="zx_pr_info"><b>Parte de los puntos de Fontanería y permite corregir la propuesta.</b><span>Los aparatos con desagüe se proponen automáticamente. Puedes añadir puntos que no tengan suministro, como sumideros, condensados, seguridad ACS o vaciados.</span></div><div class="zx_pr_section"><h3>Normativa y uso</h3><label>Uso para tabla CTE HS 5<select id="pr_drain_use"><option value="privado" ${x.uso!=="publico"?"selected":""}>Uso privado</option><option value="publico" ${x.uso==="publico"?"selected":""}>Uso público</option></select></label><label>Forma general de la red<select id="pr_drain_network"><option value="sin_indicar" ${x.tipo_red==="sin_indicar"?"selected":""}>Sin indicar</option><option value="ramales_bajante" ${x.tipo_red==="ramales_bajante"?"selected":""}>Ramales a bajante / colector</option><option value="bote_sifonico" ${x.tipo_red==="bote_sifonico"?"selected":""}>Botes sifónicos + ramales</option><option value="mixta" ${x.tipo_red==="mixta"?"selected":""}>Mixta</option></select></label><div class="zx_pr_norm_box"><b>CTE DB HS 5 · Evacuación de aguas</b><span>Las UD y diámetros mínimos individuales se toman de la tabla 4.1. Los ramales individuales de más de 1,5 m y los colectores requieren cálculo con longitud, pendiente y caudal.</span></div></div><div class="zx_pr_section"><div class="zx_pr_section_head"><div><h3>Puntos de saneamiento</h3><span>Automáticos desde Fontanería + puntos manuales</span></div><button id="pr_drain_sync" class="zx_pr_small_primary" type="button">↻ Actualizar desde Fontanería</button></div><div id="pr_drain_points" class="zx_pr_plumb_points">${pts.length?pts.map(filaSaneamientoHTML).join(""):`<div class="zx_pr_empty" data-pr-drain-empty>No hay puntos. Puedes generarlos desde Fontanería o añadir uno manual.</div>`}</div><button id="pr_drain_add" class="zx_pr_secondary_full" type="button">＋ Añadir punto manual</button></div><div class="zx_pr_section"><h3>Resultados y esquema</h3><div id="pr_drain_results"></div><div id="pr_drain_scheme"></div></div><label>Notas técnicas<textarea id="pr_drain_notes" rows="4" placeholder="Bajantes, pendientes, ventilación, arquetas, recorridos…">${limpiar(x.notas||"")}</textarea></label>`);const pintar=()=>{const pts2=leerSaneamientoFormulario(),uso=m.querySelector("#pr_drain_use").value,res=resumenSaneamiento(pts2,uso);m.querySelector("#pr_drain_results").innerHTML=`<div class="zx_pr_plumb_summary"><div><span>Puntos</span><b>${res.puntos} ud</b></div><div><span>UD calculadas</span><b>${fmtFontaneria(res.ud,0,1)} UD</b></div><div><span>Pendientes</span><b>${res.pendientes}</b></div></div><div class="zx_pr_norm_box"><b>Dimensionado de colectores pendiente</b><span>Esta primera fase fija aparatos, UD y diámetros mínimos individuales. Para dimensionar ramales, bajantes y colectores se añadirán pendiente, longitud y ventilación.</span></div>`;m.querySelector("#pr_drain_scheme").innerHTML=esquemaSaneamientoSVG({puntos:pts2,uso})};const enlazar=()=>{m.querySelectorAll("[data-pr-drain-id]").forEach(r=>{if(!r.dataset.bound){r.dataset.bound="1";conectarSaneamientoPunto(r)}});pintar()};m.querySelector("#pr_drain_back").onclick=()=>{cerrarModal();abrirFicha(p.id)};m.querySelector("#pr_drain_use").onchange=()=>{m.querySelectorAll("[data-pr-drain-id]").forEach(pintarSaneamientoPunto);pintar()};m.querySelector("#pr_drain_network").onchange=pintar;m.querySelector("#pr_drain_add").onclick=()=>{const box=m.querySelector("#pr_drain_points"),v=box.querySelector("[data-pr-drain-empty]");if(v)v.remove();box.insertAdjacentHTML("beforeend",filaSaneamientoHTML({origen:"manual",cantidad:1}));enlazar()};m.querySelector("#pr_drain_sync").onclick=()=>{const nuevos=generarSaneamientoDesdeFontaneria(p,leerSaneamientoFormulario()),box=m.querySelector("#pr_drain_points");box.innerHTML=nuevos.length?nuevos.map(filaSaneamientoHTML).join(""):`<div class="zx_pr_empty" data-pr-drain-empty>No hay puntos de Fontanería con desagüe asociado.</div>`;enlazar()};m.querySelector("#pr_drain_save").onclick=()=>guardarSaneamiento(p);enlazar()}
async function guardarSaneamiento(p){if(!sb()||!navigator.onLine){alert("Necesitas conexión para guardar el saneamiento.");return}const puntos=leerSaneamientoFormulario();if(!puntos.length){alert("Añade o genera al menos un punto de saneamiento.");return}const uso=document.getElementById("pr_drain_use")?.value||"privado",res=resumenSaneamiento(puntos,uso),ctx=contextoNormativoExtraccion(p),u=sesion(),ahora=new Date().toISOString(),meta=p.inmueble_meta&&typeof p.inmueble_meta==="object"&&!Array.isArray(p.inmueble_meta)?{...p.inmueble_meta}:{};meta.saneamiento={version:1,uso,tipo_red:document.getElementById("pr_drain_network")?.value||"sin_indicar",puntos,total_ud:res.ud,pendientes:res.pendientes,normativa:{estado:ctx.esEspana?"base_hs5_registrada_dimensionado_pendiente":"normativa_pendiente",ruleset:NORMATIVA_SANEAMIENTO_ES.ruleset,verificado_el:NORMATIVA_SANEAMIENTO_ES.verificado_el,pais:ctx.pais,provincia:ctx.provincia,municipio:ctx.municipio,cte:NORMATIVA_SANEAMIENTO_ES.cte},notas:document.getElementById("pr_drain_notes")?.value.trim()||null,actualizado_at:ahora,actualizado_por:u.nombre||u.usuario||""};const btn=document.getElementById("pr_drain_save");btn.disabled=true;btn.textContent="Guardando…";try{const q=await sb().from(TABLA).update({inmueble_meta:meta,updated_at:ahora}).eq("id",p.id).select("*").single();if(q.error)throw q.error;try{await sb().from("proyectos_historial").insert([{proyecto_id:p.id,tipo:"saneamiento",accion:"editado",resumen:`Saneamiento actualizado · ${res.puntos} punto(s) · ${res.ud} UD`,usuario_id:u.id||null,usuario:u.nombre||u.usuario||"",datos:{puntos:res.puntos,ud:res.ud,pendientes:res.pendientes,uso}}])}catch(e){}const i=CACHE.findIndex(x=>String(x.id)===String(p.id));if(i>=0){CACHE[i]=q.data;guardarCache(CACHE)}cerrarModal();abrirFicha(p.id)}catch(e){alert("No se pudo guardar el saneamiento.\n"+(e&&e.message?e.message:""));btn.disabled=false;btn.textContent="Guardar saneamiento"}}
...
function instalarCSSFontSanV4(){if(document.getElementById("zx_font_san_v4_css"))return;const s=document.createElement("style");s.id="zx_font_san_v4_css";s.textContent=`.zx_pr_scheme_wrap{margin-top:10px;border:1px solid #dbe3ef;border-radius:16px;background:#fff;overflow:auto;-webkit-overflow-scrolling:touch}.zx_pr_scheme{display:block;width:100%;min-width:720px;height:auto}.zx_pr_scheme text{font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Arial,sans-serif}.zx_pr_section #pr_water_acs_temp_status{margin-top:10px}.zx_pr_section #pr_drain_scheme{margin-top:10px}@media(max-width:620px){.zx_pr_scheme_wrap{margin-left:-4px;margin-right:-4px}.zx_pr_scheme{min-width:760px}}`;document.head.appendChild(s)}
