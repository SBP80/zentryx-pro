...
function materialTecnicoActivo(m){return tecnicoMaterial(m).equipo_termico===true}
function nombreMaterialTecnico(m){const t=tecnicoMaterial(m);return [materialTexto(m)||m&&m.nombre,textoGenerador(t.tipo_generador),t.potencia_calefaccion_kw!=null?t.potencia_calefaccion_kw+" kW":null].filter(Boolean).join(" · ")}
function opcionesCatalogoTecnico(sel){const xs=MATERIALES.filter(materialTecnicoActivo);return `<option value="">Sin equipo del catálogo técnico</option>`+xs.map(m=>`<option value="${limpiar(m.id)}" ${String(sel)===String(m.id)?"selected":""}>${limpiar(nombreMaterialTecnico(m))}</option>`).join("")}
...
function abrirCatalogoTecnico(){
  ...
  const m=modal(`<div class="zx_pr_top_actions"><button id="pr_cat_back" type="button">← Volver</button><button type="button" disabled>Catálogo técnico</button></div><div class="zx_pr_form_head"><span>EQUIPOS Y DATOS TÉCNICOS</span><h2>Catálogo técnico</h2></div><div class="zx_pr_info"><b>Usa los artículos ya existentes en Materiales.</b><span>Aquí solo se guardan sus datos técnicos. Los proyectos copian una foto fija de esos datos para que un cambio posterior del catálogo no altere proyectos anteriores.</span></div><div class="zx_pr_tools zx_pr_cat_tools"><input id="pr_cat_search" type="search" placeholder="Buscar artículo, marca, modelo o referencia"><select id="pr_cat_filter"><option value="todos">Todos</option><option value="tecnicos">Con ficha técnica</option><option value="pendientes">Sin ficha técnica</option></select></div><div class="zx_pr_cat_stats"><b>${MATERIALES.filter(materialTecnicoActivo).length}</b><span>equipos con ficha técnica de ${MATERIALES.length} artículos activos</span></div><div id="pr_cat_list" class="zx_pr_cat_list">${listaCatalogoTecnicoHTML(MATERIALES)}</div>`);
  const pintar=()=>{const q=normalizar(m.querySelector("#pr_cat_search").value),f=m.querySelector("#pr_cat_filter").value;let xs=MATERIALES.slice();if(f==="tecnicos")xs=xs.filter(materialTecnicoActivo);if(f==="pendientes")xs=xs.filter(x=>!materialTecnicoActivo(x));if(q)xs=xs.filter(x=>normalizar([x.nombre,x.marca,x.modelo,x.referencia,x.familia,textoGenerador(tecnicoMaterial(x).tipo_generador),tecnicoMaterial(x).subtipo].join(" ")).includes(q));m.querySelector("#pr_cat_list").innerHTML=listaCatalogoTecnicoHTML(xs);m.querySelectorAll("[data-pr-cat-edit]").forEach(b=>b.onclick=()=>{const x=MATERIALES.find(z=>String(z.id)===String(b.dataset.prCatEdit));if(x)formularioMaterialTecnico(x)})};
...
function formularioMaterialTecnico(mat){
  ...
  todos.onchange=()=>{checks.forEach(x=>x.checked=todos.checked);todos.indeterminate=false};
  checks.forEach(x=>x.onchange=sincronizarTodos);sincronizarTodos();
...
async function guardarMaterialTecnico(mat){
  ...
  const anterior=tecnicoMaterial(mat);const nuevo=Object.assign({},anterior,{equipo_termico:document.getElementById("pr_cat_activo").checked,tipo_generador:document.getElementById("pr_cat_tipo").value,subtipo:document.getElementById("pr_cat_subtipo").value.trim()||null,funciones:[...document.querySelectorAll("[data-pr-cat-func]:checked")].map(x=>x.dataset.prCatFunc),potencia_calefaccion_kw:numModal("pr_cat_kw_cal"),potencia_refrigeracion_kw:numModal("pr_cat_kw_ref"),rendimiento_pct:numModal("pr_cat_rend"),temperatura_impulsion_max_c:numModal("pr_cat_tmax"),cop:numModal("pr_cat_cop"),scop:numModal("pr_cat_scop"),eer:numModal("pr_cat_eer"),seer:numModal("pr_cat_seer"),foto_comercial_url:document.getElementById("pr_cat_foto").value.trim()||null,descripcion_cliente:document.getElementById("pr_cat_desc").value.trim()||null,notas:document.getElementById("pr_cat_notas").value.trim()||null,actualizado_at:new Date().toISOString(),actualizado_por:sesion().nombre||sesion().usuario||""});
...
function formularioGenerador(p,g){
  ...
  const m=modal(`<div class="zx_pr_top_actions"><button id="pr_gen_back" type="button">← Volver</button><button id="pr_gen_save" class="primary" type="button">Guardar</button></div>
    ...
    <label>Equipo del catálogo técnico<select id="pr_gen_catalogo">${opcionesCatalogoTecnico(catalogoId)}</select></label>
    <div id="pr_gen_catalogo_info" class="zx_pr_rule_gen_info ${catalogoId?"":"is-empty"}">${catalogoId?"Datos copiados desde el catálogo técnico.":"Opcional · selecciona un equipo para rellenar sus datos."}</div>
...
async function abrirFicha(id){
  ...
  const m=modal(`<div class="zx_pr_top_actions"><button id="pr_ficha_back" type="button">← Volver</button>${puedeEditar()?`<button id="pr_ficha_edit" class="primary" type="button">✏️ Editar</button>`:"<button id=\"pr_ficha_close\" type=\"button\">Cerrar</button>"}</div>
    ...
    <section class="zx_pr_next"><b>Siguiente bloque</b><span>El flujo desde la propuesta aceptada hasta Trabajo ya está preparado. El siguiente paso es usar las fichas del catálogo técnico como base de selección y después ampliar los cálculos.</span></section>`);
