// ===============================
// ZENTRYX PRO - PROYECTOS V1017
// V1017 - DOSIER COMERCIAL CONFIGURABLE Y SNAPSHOT DE ENVÍO
// ===============================
(function(){
"use strict";
...
function empresaDosier(){
  ...
  const e=cfg.empresa||{};
  return {nombre:e.nombre||"Zentryx PRO",logo:e.logo||"",color:e.color||"#0f766e",sector:e.sector||""};
}
...
function dossierPrecioHTML(xs,cfg,t){
  ...
  if(cfg.modo_precios==="capitulos"){
    ...
    return `<section class="zx_pr_dos_section"><div class="zx_pr_dos_kicker">INVERSIÓN</div><h3>Presupuesto por capítulos</h3><div class="zx_pr_dos_chapters">${cs.map(c=>`<div><span>${limpiar(c.nombre)}</span><b>${eur(c.total)}</b></div>`).join("")}</div><div class="zx_pr_dos_total"><span>Total presupuesto</span><b>${eur(t.total)}</b><small>Impuestos incluidos</small></div></section>`;
  }
...
function dossierReglasHTML(rs){
  if(!rs.length)return "";
  return `<section class="zx_pr_dos_section"><div class="zx_pr_dos_kicker">CÓMO FUNCIONARÁ</div><h3>Funcionamiento previsto</h3><div class="zx_pr_dos_flow">${rs.map((r,i)=>{let frase=textoServicioRegla(r.servicio)+" · "+textoAccionRegla(r.accion)+" · "+textoCondicionRegla(r.condicion_tipo);if(r.condicion_tipo==="fallo_generador"&&r.generador_referencia_id)frase+=" · si falla "+nombreGeneradorRelacionadoRegla(r);return `<div><span>${i+1}</span><div><b>${limpiar(nombreGeneradorRegla(r))}</b><p>${limpiar(frase)}</p>${r.notas?`<small>${limpiar(r.notas)}</small>`:""}</div></div>`}).join("")}</div></section>`;
}
function dossierEquiposHTML(gs){
  const visibles=(gs||[]).filter(g=>g.existente===false||g.se_mantiene!==false);
  if(!visibles.length)return "";
  return `<section class="zx_pr_dos_section"><div class="zx_pr_dos_kicker">EQUIPOS</div><h3>Equipos de la solución</h3><div class="zx_pr_dos_equips">${visibles.map(g=>`<article><span>${g.existente===false?"Equipo previsto":"Equipo existente"}</span><h4>${limpiar(textoGenerador(g.tipo))}</h4><b>${limpiar([g.marca,g.modelo].filter(Boolean).join(" · ")||"Sin marca/modelo")}</b><p>${g.potencia_kw!=null&&g.potencia_kw!==""?limpiar(g.potencia_kw)+" kW":"Potencia sin indicar"}${g.rol_previsto?" · "+limpiar(textoRol(g.rol_previsto)):""}</p></article>`).join("")}</div></section>`;
}
...
async function abrirVistaDosier(p,opId){
  ...
  const firma=c.mostrar_firma?`<section class="zx_pr_dos_accept"><span>SIGUIENTE PASO</span><h3>${limpiar(c.cierre||"Confirmar la opción elegida")}</h3><div><p>Cliente / firma</p><p>Fecha</p></div></section>`:"";
  const m=modal(`<div class="zx_pr_top_actions"><button id="pr_dview_back" type="button">← Volver</button>${estadoPropuesta(op)==="borrador"?`<button id="pr_dview_config" class="primary" type="button">⚙️ Configurar</button>`:`<button type="button" disabled>${estadoPropuesta(op)==="aceptada"?"Aceptada":"Enviada"}</button>`}</div>
    <div class="zx_pr_dos_wrap zx_pr_dos_${limpiar(c.estilo)}" style="--dos-primary:${pri};--dos-accent:${acc}">
      <section class="zx_pr_dos_cover">${portadaFoto}<div class="zx_pr_dos_cover_overlay"></div><div class="zx_pr_dos_brand">${d.emp.logo?`<img src="${limpiar(d.emp.logo)}" alt="Logo" onerror="this.style.display='none'">`:""}<b>${limpiar(d.emp.nombre||"Zentryx PRO")}</b></div><div class="zx_pr_dos_cover_text">${c.recomendada?`<span class="zx_pr_dos_badge">${limpiar(c.etiqueta||"Propuesta recomendada")}</span>`:""}<small>PROPUESTA COMERCIAL</small><h1>${limpiar(c.titulo)}</h1><p>${limpiar(c.subtitulo||d.descripcion||d.proyecto)}</p><div>${limpiar(d.cliente)} · ${limpiar(fechaES(new Date().toISOString()))}</div></div></section>
      ...
      ${d.descripcion?`<section class="zx_pr_dos_section zx_pr_dos_solution"><div class="zx_pr_dos_kicker">SOLUCIÓN PROPUESTA</div><h3>${limpiar(d.descripcion)}</h3></section>`:""}
      ${c.mostrar_funcionamiento?dossierReglasHTML(d.reglas):""}
      ${dossierPrecioHTML(d.partidas,c,t)}
      ${infoHTML}
      ${firma}
      <footer class="zx_pr_dos_footer"><b>${limpiar(d.emp.nombre||"Zentryx PRO")}</b><span>${limpiar(d.proyecto||op.nombre)}</span>${d.snapshot?`<small>Versión comercial guardada</small>`:"<small>Vista previa</small>"}</footer>
    </div>`);
