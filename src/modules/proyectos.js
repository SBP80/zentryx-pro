...
function cabeceraEsquemaSaneamiento(tipo){const t=normalizarTipoRedSaneamiento(tipo);return ({sin_indicar:{titulo:"Red por definir",sub:"Topología pendiente"},sifones_individuales:{titulo:"Ramales / bajante-colector",sub:"Sifones individuales"},bote_sifonico:{titulo:"Bote(s) sifónico(s) / ramal",sub:"Agrupación por local húmedo"},horizontal_directa:{titulo:"Colector horizontal / arqueta",sub:"Sin bajante obligatoria"},varias_zonas:{titulo:"Varios ramales / colectores",sub:"Por zonas o plantas"},mixta:{titulo:"Red mixta",sub:"Combinación de soluciones"},personalizada:{titulo:"Red personalizada",sub:"Definición técnica"}})[t]||{titulo:"Red por definir",sub:"Topología pendiente"}}
function esquemaSaneamientoSVG(x){x=x||{};const pts=Array.isArray(x.puntos)?x.puntos:[],uso=x.uso||"privado",tipo=normalizarTipoRedSaneamiento(x.tipo_red),cab=cabeceraEsquemaSaneamiento(tipo),h=Math.max(360,220+Math.max(1,pts.length)*68),collectorX=92,pointX=158,y0=146,step=68;let rows="";pts.forEach((pt,i)=>{const y=y0+i*step,d=defSaneamiento(pt.tipo),rd=d?(uso==="publico"?d.d_pub:d.d_priv):null,diam=rd?`Ø${rd}`:"Ø?",nombre=pt.nombre||((d?d.nombre:pt.tipo)+(pt.cantidad>1?" ×"+pt.cantidad:"")),detalle=[pt.ubicacion,pt.ramal].filter(Boolean).join(" · ")||"Sin zona / ramal";rows+=`<line x1="${collectorX}" y1="${y}" x2="${pointX}" y2="${y}" stroke="#7c5a3b" stroke-width="4"/><rect x="106" y="${y-12}" width="44" height="22" rx="11" fill="#f3e8dc"/><text x="113" y="${y+3}" font-size="9" font-weight="700" fill="#6b4423">${limpiar(diam)}</text><rect x="${pointX}" y="${y-24}" width="186" height="50" rx="10" fill="#fff" stroke="#cbd5e1"/><text x="${pointX+10}" y="${y-6}" font-size="11" font-weight="700" fill="#0f172a">${limpiar(nombre).slice(0,28)}</text><text x="${pointX+10}" y="${y+10}" font-size="9" fill="#64748b">${limpiar(detalle).slice(0,34)}</text><text x="${pointX+10}" y="${y+22}" font-size="8" fill="#7c5a3b">${limpiar((d&&d.nombre)||pt.tipo).slice(0,34)}</text>`});const endY=y0+Math.max(0,pts.length-1)*step+44,outY=h-62;return `<div class="zx_pr_scheme_wrap zx_pr_scheme_wrap_drain"><svg class="zx_pr_scheme zx_pr_scheme_drain" viewBox="0 0 360 ${h}" role="img" aria-label="Esquema de saneamiento"><rect width="360" height="${h}" rx="18" fill="#fff"/><text x="16" y="28" font-size="15" font-weight="800" fill="#071330">Esquema de saneamiento</text><rect x="16" y="42" width="132" height="24" rx="12" fill="#f3e8dc"/><text x="29" y="58" font-size="10" font-weight="700" fill="#6b4423">Aguas residuales</text><rect x="18" y="82" width="148" height="48" rx="10" fill="#f3e8dc" stroke="#7c5a3b"/><text x="31" y="102" font-size="10.5" font-weight="700" fill="#4a2f1a">${limpiar(cab.titulo).slice(0,25)}</text><text x="31" y="118" font-size="8.2" fill="#6b4423">${limpiar(cab.sub).slice(0,29)}</text><line x1="${collectorX}" y1="130" x2="${collectorX}" y2="${Math.max(endY,outY)}" stroke="#7c5a3b" stroke-width="6"/>${rows}<path d="M ${collectorX} ${outY} H 42" stroke="#7c5a3b" stroke-width="6" fill="none"/><rect x="16" y="${outY-22}" width="112" height="42" rx="10" fill="#e2e8f0" stroke="#64748b"/><text x="29" y="${outY+3}" font-size="10" font-weight="700" fill="#334155">Salida / arqueta</text><text x="16" y="${h-15}" font-size="8.5" fill="#64748b">Esquema inicial · dimensionado detallado pendiente.</text></svg></div>`}
...
async function abrirFicha(id){
  ...
  const m=modal(`<div class="zx_pr_top_actions"><button id="pr_ficha_back" type="button">← Volver</button>${puedeEditar()?`<button id="pr_ficha_edit" class="primary" type="button">✏️ Editar</button>`:"<button id=\"pr_ficha_close\" type=\"button\">Cerrar</button>"}</div>
    ...
    <section class="zx_pr_next"><b>Proyectos multidisciplinares</b><span>Climatización dispone de cálculo técnico orientativo por estancias; Fontanería registra suministro, ACS, topología y esquema; Saneamiento puede proponer desagües desde los puntos de consumo y después editarse; Extracción calcula zonas, caudales, conductos, pérdidas y compara extractores del catálogo técnico. Electricidad, ventilación, aire acondicionado y control de humedad quedan para sus siguientes bloques técnicos.</span></section>`);
