...
function metaFontaneriaProyecto(p){
  ...
  const acometida=Object.assign({},aDef,aRaw);
  ...
  return Object.assign({version:2,modo_asistente:"normativa",puntos:[],tecnico:tec,acometida,notas:""},x,{puntos:Array.isArray(x.puntos)?x.puntos:[],tecnico:tec,acometida});
}
...
function leerPuntosFontaneriaFormulario(incluirVacios=false){
  return [...document.querySelectorAll(".zx_pr_plumb_point")].map(r=>({punto_id:r.dataset.prWaterId||uidFontaneria(),tipo:String(r.querySelector("[data-pr-water-type]")?.value||""),cantidad:Math.max(0,Math.round(nExt(r.querySelector("[data-pr-water-qty]")?.value,0))),ubicacion:String(r.querySelector("[data-pr-water-room]")?.value||"").trim()})).filter(x=>incluirVacios||x.tipo&&x.cantidad>0)
}
function resumenFontaneriaDesdePuntos(puntos){
  let af=0,acs=0,uds=0;const detalle=[];(puntos||[]).forEach(p=>{const d=defPuntoFontaneria(p.tipo),q=Math.max(0,Math.round(nExt(p.cantidad)));if(!d||!q)return;uds+=q;af+=d.af*q;acs+=d.acs*q;detalle.push({punto:p,def:d,af:d.af*q,acs:d.acs*q})});return {af_ls:redondearCalc(af,3),acs_ls:redondearCalc(acs,3),unidades:uds,detalle}
}
...
function actualizarResumenFontaneriaFormulario(){
  const modo=document.getElementById("pr_water_mode")?.value||"normativa",tech=document.getElementById("pr_water_tech"),tag=document.getElementById("pr_water_mode_tag"),info=document.getElementById("pr_water_mode_info");if(tech)tech.hidden=modo!=="avanzado";if(tag)tag.textContent=modo==="avanzado"?"FONTANERÍA · TÉCNICO":"FONTANERÍA · ASISTENTE GUIADO";if(info){info.querySelector("b").textContent=modo==="avanzado"?"Dimensionado técnico de la red":"Registra los puntos de consumo";info.querySelector("span").textContent=modo==="avanzado"?"Introduce simultaneidad, material, velocidad, presión dinámica y pérdidas del proyecto. Zentryx calcula caudales de cálculo, diámetro interior teórico y una comprobación de presión.":"Selecciona los aparatos y cantidades. Añade los datos sencillos de la acometida si los conoces; Zentryx no deduce un suministro cierto únicamente por el diámetro."}
  const puntos=leerPuntosFontaneriaFormulario(),base=resumenFontaneriaDesdePuntos(puntos),a=leerAcometidaFormulario(),tec=calculoTecnicoFontaneria(base,datosTecnicosFontaneriaFormulario(a)),box=document.getElementById("pr_water_results");if(box)box.innerHTML=resumenFontaneriaHTML(base,tec,modo);pintarEstadoAcometidaFormulario();const acs=document.getElementById("pr_water_acs_sources");if(acs)acs.innerHTML=produccionACSProyectoHTML(base)
}
...
async function abrirFicha(id){
  ...
  const m=modal(`<div class="zx_pr_top_actions"><button id="pr_ficha_back" type="button">← Volver</button>${puedeEditar()?`<button id="pr_ficha_edit" class="primary" type="button">✏️ Editar</button>`:"<button id=\"pr_ficha_close\" type=\"button\">Cerrar</button>"}</div>
    ...
    <section class="zx_pr_next"><b>Proyectos multidisciplinares</b><span>Climatización dispone de cálculo técnico orientativo por estancias, Fontanería ya registra puntos de consumo y una base de dimensionado CTE DB HS 4, y Extracción calcula zonas, caudales, conductos, pérdidas y compara extractores del catálogo técnico. Electricidad, ventilación, aire acondicionado y control de humedad quedan preparadas para sus siguientes bloques técnicos.</span></section>`);
