...
function montarAsistenteRecorridoSaneamiento(m,repintar){const panel=m.querySelector("#pr_drain_route_assistant"),start=m.querySelector("#pr_drain_route_assistant_start");if(!panel||!start)return;let borrador=null,pasos=[],i=0;const box=()=>m.querySelector("#pr_drain_network_elements"),cerrar=()=>{panel.hidden=true;panel.innerHTML="";borrador=null;pasos=[];i=0};const guardarValor=()=>{const paso=pasos[i],inp=panel.querySelector("[data-pr-route-wiz-input]");if(!paso||!inp)return;const st=borrador.estado_campos||{};let valor;if(paso.kind==="number")valor=numFontOpt(inp.value);else valor=String(inp.value||"").trim();borrador[paso.k]=valor;const marcado=st[paso.k],vacio=paso.kind==="number"?valor==null:["material","montaje"].includes(paso.kind)?valor==="sin_indicar":valor==="";if(!(marcado&&vacio))delete st[paso.k];borrador.estado_campos=st};const renderPaso=()=>{const paso=pasos[i],pref=borrador[paso.k],st=borrador.estado_campos&&borrador.estado_campos[paso.k],t=textoTipoElementoRedSaneamiento(borrador.tipo);panel.hidden=false;panel.innerHTML=`<div class="zx_pr_route_wizard_head"><div><b>Asistente de recorrido</b><span>${limpiar(t)} · paso ${i+1} de ${pasos.length}</span></div><button type="button" data-pr-route-wiz-close aria-label="Cerrar asistente">×</button></div><div class="zx_pr_route_wizard_progress"><i style="width:${Math.round((i+1)/pasos.length*100)}%"></i></div><div class="zx_pr_route_wizard_question">${limpiar(paso.q)}</div><div class="zx_pr_route_wizard_control">${htmlControlPasoAsistenteSaneamiento(paso,borrador)}</div>${st?`<div class="zx_pr_route_wizard_mark">Marcado: ${limpiar(textoEstadoCampoElementoSaneamiento(st))}</div>`:""}<button class="zx_pr_route_help_btn" type="button" data-pr-route-wiz-help>¿Cómo obtener este dato?</button><div class="zx_pr_route_wizard_help" data-pr-route-wiz-help-box hidden>${limpiar(paso.help||"No hay indicaciones adicionales para este dato.")}</div>${paso.kind!=="estado"?`<div class="zx_pr_route_wizard_unknown"><button type="button" data-pr-route-wiz-state="no_conocido">No se conoce</button><button type="button" data-pr-route-wiz-state="mas_tarde">Lo introduciré luego</button>${paso.optional?'<button type="button" data-pr-route-wiz-state="no_aplica">No aplica</button>':""}</div>`:""}<div class="zx_pr_route_wizard_nav"><button type="button" data-pr-route-wiz-prev ${i===0?"disabled":""}>← Anterior</button><button class="primary" type="button" data-pr-route-wiz-next>${i===pasos.length-1?"Añadir al plano":"Siguiente →"}</button></div>`;panel.querySelector("[data-pr-route-wiz-close]").onclick=cerrar;panel.querySelector("[data-pr-route-wiz-help]").onclick=()=>{const h=panel.querySelector("[data-pr-route-wiz-help-box]");h.hidden=!h.hidden};panel.querySelectorAll("[data-pr-route-wiz-state]").forEach(b=>b.onclick=()=>{const paso=pasos[i],st=borrador.estado_campos||{};st[paso.k]=b.dataset.prRouteWizState;borrador.estado_campos=st;if(paso.kind==="number")borrador[paso.k]=null;else if(paso.kind==="material")borrador[paso.k]="sin_indicar";else if(paso.kind==="montaje")borrador[paso.k]="sin_indicar";else borrador[paso.k]="";if(i<pasos.length-1){i++;renderPaso()}else renderPaso()});panel.querySelector("[data-pr-route-wiz-prev]").onclick=()=>{guardarValor();if(i>0){i--;renderPaso()}};panel.querySelector("[data-pr-route-wiz-next]").onclick=()=>{guardarValor();if(i<pasos.length-1){i++;renderPaso()}else finalizar()};const inp=panel.querySelector("[data-pr-route-wiz-input]");if(inp)requestAnimationFrame(()=>{try{inp.focus({preventScroll:true})}catch(e){}})};const finalizar=()=>{const b=box(),v=b&&b.querySelector("[data-pr-drain-net-empty]");if(v)v.remove();if(!b)return;const t=document.createElement("div");t.innerHTML=filaElementoRedSaneamientoHTML(borrador);const row=t.firstElementChild;b.appendChild(row);conectarElementoRedSaneamiento(row,repintar);cerrar();repintar&&repintar();requestAnimationFrame(()=>row.scrollIntoView({behavior:"smooth",block:"start"}))};const elegirTipo=tipo=>{borrador=prellenarElementoAsistenteSaneamiento(tipo,leerDetalleRedSaneamientoFormulario());pasos=pasosAsistenteElementoSaneamiento(tipo);i=0;const conocidos=[borrador.longitud_m,borrador.diametro_mm,borrador.pendiente_pct,borrador.material!=="sin_indicar"?borrador.material:null,borrador.hasta,borrador.ubicacion].filter(x=>x!=null&&x!=="").length;renderPaso();if(conocidos){const info=document.createElement("div");info.className="zx_pr_route_wizard_prefill";info.textContent=`Zentryx ha recuperado ${conocidos} dato(s) ya registrados para este elemento. Revísalos antes de terminar.`;panel.insertBefore(info,panel.querySelector(".zx_pr_route_wizard_question"))}};const renderTipos=()=>{panel.hidden=false;panel.innerHTML=`<div class="zx_pr_route_wizard_head"><div><b>Asistente de recorrido</b><span>Un elemento cada vez</span></div><button type="button" data-pr-route-wiz-close aria-label="Cerrar asistente">×</button></div><div class="zx_pr_route_wizard_intro"><b>¿Qué elemento quieres documentar?</b><span>Zentryx reutilizará los datos que ya existan. Si falta algo podrás marcarlo como desconocido o dejarlo para después.</span></div><div class="zx_pr_route_wizard_types">${TIPOS_ELEMENTO_RED_SANEAMIENTO.map(([v,t])=>`<button type="button" data-pr-route-wiz-type="${v}">${limpiar(t)}</button>`).join("")}</div>`;panel.querySelector("[data-pr-route-wiz-close]").onclick=cerrar;panel.querySelectorAll("[data-pr-route-wiz-type]").forEach(b=>b.onclick=()=>elegirTipo(b.dataset.prRouteWizType));panel.scrollIntoView({behavior:"smooth",block:"start"})};start.onclick=renderTipos}
...
function metaSaneamientoProyecto(p){
  ...
  return Object.assign({version:8,uso:"privado",tipo_red:"sin_indicar",puntos:[],detalle_red:normalizarDetalleRedSaneamiento(),notas:""},x,{version:Math.max(8,Number(x.version)||1),tipo_red:normalizarTipoRedSaneamiento(x.tipo_red),puntos:normalizarPuntosSaneamientoV2(p,Array.isArray(x.puntos)?x.puntos:[]),detalle_red:normalizarDetalleRedSaneamiento(x.detalle_red)})
}
