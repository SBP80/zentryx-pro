...
const NORMATIVA_SANEAMIENTO_ES={ruleset:"ES-CTE-HS5-EVACUACION-AGUAS",verificado_el:"2026-09-10",cte:{nombre:"CTE · DB HS 5 Evacuación de aguas",ref:"HS 5 · tabla 4.1 y apartado 4.1.1"}};
...
function puntoSaneamientoEsquema(pt,uso){const d=defSaneamiento(pt.tipo),rd=d?(uso==="publico"?d.d_pub:d.d_priv):null,nombre=pt.nombre||((d?d.nombre:pt.tipo)+(pt.cantidad>1?" ×"+pt.cantidad:"")),detalle=[pt.ubicacion,pt.ramal].filter(Boolean).join(" · ")||"Sin zona / ramal",ud=d?(d.continuo?(pt.caudal_ls!=null?Math.max(1,Math.ceil(Number(pt.caudal_ls)/0.03)):null):(uso==="publico"?d.ud_pub:d.ud_priv)):null;return {d,rd,nombre,detalle,ud}}
function esquemaSaneamientoSVG(x){x=x||{};const pts=Array.isArray(x.puntos)?x.puntos:[],uso=x.uso||"privado",tipo=normalizarTipoRedSaneamiento(x.tipo_red),cab=cabeceraEsquemaSaneamiento(tipo),C=ZX_ESQUEMAS.colores,horizontal=tipo==="horizontal_directa";
  if(horizontal){
    const filas=Math.max(1,Math.ceil(pts.length/2)),h=Math.max(390,190+filas*118),collectorY=h-82,outX=314,mainY=collectorY;let rows="";
    pts.forEach((pt,i)=>{const info=puntoSaneamientoEsquema(pt,uso),col=i%2,row=Math.floor(i/2),x0=col?190:18,w=152,y=132+row*112,branchX=x0+w/2,diam=info.rd?`Ø${info.rd} mm`:"Ø pendiente";rows+=`<rect x="${x0}" y="${y}" width="${w}" height="64" rx="11" fill="#fff" stroke="${C.borde}"/>${zxSvgLineas(info.nombre,x0+10,y+19,20,10.5,800,C.texto,12)}<text x="${x0+10}" y="${y+45}" font-size="8.5" fill="${C.secundario}">${limpiar(info.detalle).slice(0,27)}</text><text x="${x0+10}" y="${y+57}" font-size="8.2" font-weight="700" fill="${C.saneamiento}">${limpiar(diam)}${info.ud!=null?" · "+limpiar(info.ud)+" UD":""}</text><path d="M ${branchX} ${y+64} V ${mainY}" fill="none" stroke="${C.saneamiento}" stroke-width="4"/><circle cx="${branchX}" cy="${mainY}" r="5" fill="${C.saneamiento}"/>`});
    return `<div class="zx_pr_scheme_wrap zx_pr_scheme_wrap_drain"><svg class="zx_pr_scheme zx_pr_scheme_drain" viewBox="0 0 360 ${h}" role="img" aria-label="Esquema técnico de saneamiento"><defs><marker id="zxDrainArrowH" markerWidth="7" markerHeight="7" refX="6" refY="3.5" orient="auto"><path d="M0,0 L7,3.5 L0,7 Z" fill="${C.saneamiento}"/></marker></defs><rect width="360" height="${h}" rx="18" fill="#fff"/><text x="16" y="28" font-size="15" font-weight="800" fill="${C.texto}">Esquema técnico · saneamiento</text><rect x="16" y="42" width="116" height="24" rx="12" fill="${C.saneamientoSuave}"/><text x="28" y="58" font-size="9.5" font-weight="800" fill="${C.saneamiento}">Red residual</text><rect x="16" y="78" width="328" height="44" rx="11" fill="#f8fafc" stroke="${C.borde}"/>${zxSvgLineas(cab.titulo,28,97,34,10.5,800,C.texto,12)}<text x="28" y="113" font-size="8.5" fill="${C.secundario}">${limpiar(cab.sub)}</text>${rows}<line x1="30" y1="${mainY}" x2="${outX-16}" y2="${mainY}" stroke="${C.saneamiento}" stroke-width="6" marker-end="url(#zxDrainArrowH)"/><text x="30" y="${mainY-10}" font-size="9" font-weight="800" fill="${C.saneamiento}">Colector horizontal · pendiente/Ø por calcular</text><rect x="${outX-54}" y="${mainY-24}" width="82" height="48" rx="10" fill="#ccfbf1" stroke="${C.salida}"/><text x="${outX-43}" y="${mainY-4}" font-size="9" font-weight="800" fill="${C.salida}">Salida</text><text x="${outX-43}" y="${mainY+10}" font-size="8" fill="${C.salida}">arqueta / red</text><text x="16" y="${h-18}" font-size="8.2" fill="${C.secundario}">Esquema de trabajo · recorridos y cotas detalladas pendientes.</text></svg></div>`;
  }
  const h=Math.max(390,232+Math.max(1,pts.length)*72),trunkX=82,pointX=142,y0=154,step=72,outY=h-68;let rows="";
  pts.forEach((pt,i)=>{const y=y0+i*step,info=puntoSaneamientoEsquema(pt,uso),diam=info.rd?`Ø${info.rd}`:"Ø?";rows+=`<path d="M ${trunkX} ${y} H ${pointX}" fill="none" stroke="${C.saneamiento}" stroke-width="4"/><circle cx="${trunkX}" cy="${y}" r="5" fill="${C.saneamiento}"/><rect x="96" y="${y-12}" width="40" height="22" rx="11" fill="${C.saneamientoSuave}"/><text x="104" y="${y+3}" font-size="8.8" font-weight="800" fill="${C.saneamiento}">${limpiar(diam)}</text><rect x="${pointX}" y="${y-27}" width="202" height="56" rx="10" fill="#fff" stroke="${C.borde}"/>${zxSvgLineas(info.nombre,pointX+10,y-8,27,10.5,800,C.texto,12)}<text x="${pointX+10}" y="${y+11}" font-size="8.5" fill="${C.secundario}">${limpiar(info.detalle).slice(0,34)}</text><text x="${pointX+10}" y="${y+24}" font-size="8" font-weight="700" fill="${C.saneamiento}">${limpiar((info.d&&info.d.nombre)||pt.tipo).slice(0,24)}${info.ud!=null?" · "+limpiar(info.ud)+" UD":""}</text>`});
  return `<div class="zx_pr_scheme_wrap zx_pr_scheme_wrap_drain"><svg class="zx_pr_scheme zx_pr_scheme_drain" viewBox="0 0 360 ${h}" role="img" aria-label="Esquema técnico de saneamiento"><defs><marker id="zxDrainArrowV" markerWidth="7" markerHeight="7" refX="6" refY="3.5" orient="auto"><path d="M0,0 L7,3.5 L0,7 Z" fill="${C.saneamiento}"/></marker></defs><rect width="360" height="${h}" rx="18" fill="#fff"/><text x="16" y="28" font-size="15" font-weight="800" fill="${C.texto}">Esquema técnico · saneamiento</text><rect x="16" y="42" width="116" height="24" rx="12" fill="${C.saneamientoSuave}"/><text x="28" y="58" font-size="9.5" font-weight="800" fill="${C.saneamiento}">Red residual</text><rect x="16" y="78" width="328" height="54" rx="11" fill="#f8fafc" stroke="${C.borde}"/>${zxSvgLineas(cab.titulo,28,98,36,10.5,800,C.texto,12)}<text x="28" y="122" font-size="8.5" fill="${C.secundario}">${limpiar(cab.sub)}</text><line x1="${trunkX}" y1="132" x2="${trunkX}" y2="${outY-4}" stroke="${C.saneamiento}" stroke-width="6" marker-end="url(#zxDrainArrowV)"/>${rows}<rect x="24" y="${outY-22}" width="116" height="46" rx="10" fill="#ccfbf1" stroke="${C.salida}"/><text x="38" y="${outY-2}" font-size="9.5" font-weight="800" fill="${C.salida}">Salida / arqueta</text><text x="38" y="${outY+12}" font-size="8" fill="${C.salida}">${tipo==="sifones_individuales"?"desde colector/bajante":"red de evacuación"}</text><text x="16" y="${h-16}" font-size="8.2" fill="${C.secundario}">Esquema de trabajo · dimensionado detallado pendiente.</text></svg></div>`}
function bloqueSaneamientoFichaHTML(p){if(!especialidadesProyecto(p).includes("saneamiento"))return "";const x=metaSaneamientoProyecto(p),font=metaFontaneriaProyecto(p),propuestos=generarSaneamientoDesdeFontaneria(p,x.puntos),pts=x.puntos.length?x.puntos:propuestos,res=resumenSaneamiento(pts,x.uso,x.tipo_red);return `<section class="zx_pr_section"><div class="zx_pr_section_head"><div><h3>Saneamiento · aguas residuales</h3><span>${pts.length?res.puntos+" punto(s) · "+fmtFontaneria(res.ud,0,1)+" UD":"Sin puntos guardados"}</span></div>${puedeEditar()?`<button id="pr_drain_edit" class="zx_pr_small_primary" type="button">${x.puntos.length?"Editar saneamiento":"Configurar saneamiento"}</button>`:""}</div>${!x.puntos.length&&font.puntos.length?`<div class="zx_pr_norm_box"><b>Propuesta disponible desde Fontanería</b><span>Zentryx crea un punto de saneamiento independiente por cada aparato real, aunque en Fontanería se haya indicado una cantidad mayor que 1. Después puedes nombrarlos, asignarlos a estancias/ramales y añadir sumideros, condensados, descargas de seguridad, vaciados y otros puntos.</span></div>`:""}${pts.length?`<div class="zx_pr_plumb_summary"><div><span>Puntos previstos</span><b>${res.puntos} ud</b></div><div><span>Unidades de desagüe</span><b>${fmtFontaneria(res.ud,0,1)} UD</b></div><div><span>Dimensionado individual</span><b>${res.completos} ${res.completos===1?"completo":"completos"} · ${res.pendientes_tecnicos} ${res.pendientes_tecnicos===1?"pendiente":"pendientes"}</b></div><div><span>Red</span><b>${limpiar(res.red_estado)}</b></div></div>${esquemaSaneamientoSVG(Object.assign({},x,{puntos:pts}))}`:`<div class="zx_pr_empty">Saneamiento está activo, pero todavía no hay puntos propuestos o añadidos.</div>`}</section>`}
function formularioSaneamiento(p){instalarCSSFontSanV4();const x=metaSaneamientoProyecto(p),auto=!x.puntos.length&&metaFontaneriaProyecto(p).puntos.length,pts=auto?generarSaneamientoDesdeFontaneria(p,[]):x.puntos.slice(),m=modal(`<div class="zx_pr_top_actions"><button id="pr_drain_back" type="button">← Volver</button><button id="pr_drain_save" class="primary" type="button">Guardar saneamiento</button></div><div class="zx_pr_form_head"><span>SANEAMIENTO · ASISTENTE</span><h2>${limpiar(p.nombre)}</h2></div><div class="zx_pr_info"><b>Parte de los puntos de Fontanería y permite corregir la propuesta.</b><span>Cada aparato real se muestra por separado para poder darle nombre, estancia y ramal propios. También puedes añadir puntos sin suministro, como sumideros, condensados, seguridad ACS o vaciados.</span></div><div class="zx_pr_section"><h3>Normativa y uso</h3><label>Uso para tabla CTE HS 5<select id="pr_drain_use"><option value="privado" ${x.uso!=="publico"?"selected":""}>Uso privado</option><option value="publico" ${x.uso==="publico"?"selected":""}>Uso público</option></select></label><label>Forma general de la red<select id="pr_drain_network">${opcionesTipoRedSaneamiento(x.tipo_red)}</select></label><div class="zx_pr_norm_box"><b>CTE DB HS 5 · Evacuación de aguas</b><span>Elige la topología real sin asumir que siempre existe bajante. Las UD y diámetros mínimos individuales se toman de la tabla 4.1. Longitudes, pendientes, colectores/bajantes y ventilación se calculan después; la ventilación se trata como dato independiente de esta selección.</span></div></div><div class="zx_pr_section"><div class="zx_pr_section_head"><div><h3>Puntos de saneamiento</h3><span>Automáticos desde Fontanería + puntos manuales</span></div><button id="pr_drain_sync" class="zx_pr_small_primary" type="button">↻ Actualizar desde Fontanería</button></div><div id="pr_drain_points" class="zx_pr_plumb_points">${pts.length?pts.map(filaSaneamientoHTML).join(""):`<div class="zx_pr_empty" data-pr-drain-empty>No hay puntos. Puedes generarlos desde Fontanería o añadir uno manual.</div>`}</div><button id="pr_drain_add" class="zx_pr_secondary_full" type="button">＋ Añadir punto manual</button></div><div class="zx_pr_section"><h3>Resultados y esquema</h3><div id="pr_drain_results"></div><div id="pr_drain_scheme"></div></div><label>Notas técnicas<textarea id="pr_drain_notes" rows="4" placeholder="Bajantes, pendientes, ventilación, arquetas, recorridos…">${limpiar(x.notas||"")}</textarea></label>`);const pintar=()=>{const pts2=leerSaneamientoFormulario(),uso=m.querySelector("#pr_drain_use").value,tipoRed=m.querySelector("#pr_drain_network").value,res=resumenSaneamiento(pts2,uso,tipoRed);m.querySelector("#pr_drain_results").innerHTML=`<div class="zx_pr_plumb_summary"><div><span>Puntos</span><b>${res.puntos} ud</b></div><div><span>UD calculadas</span><b>${fmtFontaneria(res.ud,0,1)} UD</b></div><div><span>Dimensionado individual</span><b>${res.completos} ${res.completos===1?"completo":"completos"} · ${res.pendientes_tecnicos} ${res.pendientes_tecnicos===1?"pendiente":"pendientes"}</b></div><div><span>Red</span><b>${limpiar(res.red_estado)}</b></div></div><div class="zx_pr_norm_box"><b>Dimensionado de red pendiente</b><span>${limpiar(textoPendienteRedSaneamiento(tipoRed))}</span></div>`;m.querySelector("#pr_drain_scheme").innerHTML=esquemaSaneamientoSVG({puntos:pts2,uso,tipo_red:tipoRed})};const enlazar=()=>{m.querySelectorAll("[data-pr-drain-id]").forEach(r=>{if(!r.dataset.bound){r.dataset.bound="1";conectarSaneamientoPunto(r,pintar)}});pintar()};m.querySelector("#pr_drain_back").onclick=()=>{cerrarModal();abrirFicha(p.id)};m.querySelector("#pr_drain_use").onchange=()=>{m.querySelectorAll("[data-pr-drain-id]").forEach(pintarSaneamientoPunto);pintar()};m.querySelector("#pr_drain_network").onchange=pintar;m.querySelector("#pr_drain_add").onclick=()=>{const box=m.querySelector("#pr_drain_points"),v=box.querySelector("[data-pr-drain-empty]");if(v)v.remove();box.insertAdjacentHTML("beforeend",filaSaneamientoHTML({origen:"manual",cantidad:1}));enlazar()};m.querySelector("#pr_drain_sync").onclick=()=>{const nuevos=generarSaneamientoDesdeFontaneria(p,leerSaneamientoFormulario()),box=m.querySelector("#pr_drain_points");box.innerHTML=nuevos.length?nuevos.map(filaSaneamientoHTML).join(""):`<div class="zx_pr_empty" data-pr-drain-empty>No hay puntos de Fontanería con desagüe asociado.</div>`;enlazar()};m.querySelector("#pr_drain_save").onclick=()=>guardarSaneamiento(p);enlazar()}
async function guardarSaneamiento(p){if(!sb()||!navigator.onLine){alert("Necesitas conexión para guardar el saneamiento.");return}const puntos=leerSaneamientoFormulario();if(!puntos.length){alert("Añade o genera al menos un punto de saneamiento.");return}const uso=document.getElementById("pr_drain_use")?.value||"privado",tipo_red=document.getElementById("pr_drain_network")?.value||"sin_indicar",res=resumenSaneamiento(puntos,uso,tipo_red),ctx=contextoNormativoExtraccion(p),u=sesion(),ahora=new Date().toISOString(),meta=p.inmueble_meta&&typeof p.inmueble_meta==="object"&&!Array.isArray(p.inmueble_meta)?{...p.inmueble_meta}:{};meta.saneamiento={version:5,uso,tipo_red:normalizarTipoRedSaneamiento(tipo_red),puntos,total_ud:res.ud,puntos_definidos:res.completos,pendientes_datos:res.pendientes_datos,pendientes_dimensionado:res.pendientes_dimensionado,pendientes_puntos:res.pendientes_tecnicos,dimensionado_red_pendiente:true,normativa:{estado:ctx.esEspana?"base_hs5_registrada_dimensionado_pendiente":"normativa_pendiente",ruleset:NORMATIVA_SANEAMIENTO_ES.ruleset,verificado_el:NORMATIVA_SANEAMIENTO_ES.verificado_el,pais:ctx.pais,provincia:ctx.provincia,municipio:ctx.municipio,cte:NORMATIVA_SANEAMIENTO_ES.cte},notas:document.getElementById("pr_drain_notes")?.value.trim()||null,actualizado_at:ahora,actualizado_por:u.nombre||u.usuario||""};const btn=document.getElementById("pr_drain_save");btn.disabled=true;btn.textContent="Guardando…";try{const q=await sb().from(TABLA).update({inmueble_meta:meta,updated_at:ahora}).eq("id",p.id).select("*").single();if(q.error)throw q.error;try{await sb().from("proyectos_historial").insert([{proyecto_id:p.id,tipo:"saneamiento",accion:"editado",resumen:`Saneamiento actualizado · ${res.puntos} punto(s) · ${res.ud} UD`,usuario_id:u.id||null,usuario:u.nombre||u.usuario||"",datos:{puntos:res.puntos,ud:res.ud,puntos_definidos:res.completos,pendientes_datos:res.pendientes_datos,pendientes_dimensionado:res.pendientes_dimensionado,pendientes_puntos:res.pendientes_tecnicos,dimensionado_red_pendiente:true,uso,tipo_red}}])}catch(e){}const i=CACHE.findIndex(x=>String(x.id)===String(p.id));if(i>=0){CACHE[i]=q.data;guardarCache(CACHE)}cerrarModal();abrirFicha(p.id)}catch(e){alert("No se pudo guardar el saneamiento.\n"+(e&&e.message?e.message:""));btn.disabled=false;btn.textContent="Guardar saneamiento"}}
