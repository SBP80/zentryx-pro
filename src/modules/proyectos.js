...
function filaEspecificacionHTML(x){x=x||{};return `<div class="zx_pr_cat_spec_row"><input data-pr-cat-spec-nombre value="${limpiar(x.nombre||"")}" placeholder="Dato: diámetro, sección, material…"><input data-pr-cat-spec-valor value="${limpiar(x.valor??"")}" placeholder="Valor"><input data-pr-cat-spec-medida value="${limpiar(x.medida||x.unidad||"")}" placeholder="Medida: mm, V, bar…"><button type="button" data-pr-cat-spec-del aria-label="Quitar dato">×</button></div>`}
function enlazarEspecificaciones(m){m.querySelectorAll("[data-pr-cat-spec-del]").forEach(b=>b.onclick=()=>{const r=b.closest(".zx_pr_cat_spec_row");if(r)r.remove()})}
function formularioMaterialTecnico(mat){
  ...
  const ajustarClase=()=>{bloqueGen.style.display=cat.value==="generador"?"grid":"none"};
  ...
  m.querySelector("#pr_cat_spec_add").onclick=()=>{m.querySelector("#pr_cat_specs").insertAdjacentHTML("beforeend",filaEspecificacionHTML({}));enlazarEspecificaciones(m)};enlazarEspecificaciones(m);
