...
function resumenSaneamiento(puntos,uso,tipoRed="sin_indicar"){let ud=0,pend=0,uds=0;(puntos||[]).forEach(x=>{const d=defSaneamiento(x.tipo),q=Math.max(1,Math.round(nExt(x.cantidad,1)));uds+=q;if(!d){pend+=q;return}let val=null;if(d.continuo)val=x.caudal_ls!=null?Math.max(1,Math.ceil(Number(x.caudal_ls)/0.03))*q:null;else val=uso==="publico"?d.ud_pub:d.ud_priv;if(val==null)pend+=q;else ud+=d.continuo?val:val*q});const tr=normalizarTipoRedSaneamiento(tipoRed);return {puntos:uds,ud:redondearCalc(ud,1),pendientes:pend,red_estado:tr==="sin_indicar"?"Forma por definir":textoTipoRedSaneamiento(tr),red_pendiente:true}}
