...
function metaExtraccionProyecto(p){
  ...
  return Object.assign({version:3,modo_asistente:"normativa",dormitorios_grupo:"",simultaneidad_pct:100,margen_pct:10,zonas:[],sistemas:[]},x,{zonas:Array.isArray(x.zonas)?x.zonas:[],sistemas:Array.isArray(x.sistemas)?x.sistemas:[]});
}
const NORMATIVA_EXTRACCION_ES={
  ruleset:"ES-CTE-HS3-RD450-2022_RITE-RD1027-2007-20220802",
  ...
  rite:{nombre:"RITE · IT 1.1.4.2",consolidado:"última modificación 02/08/2022",ref:"IT 1.1.4.2.2 y 1.1.4.2.3"}
};
...
function textoTipoNormativoZona(v){return ({sin_clasificar:"Selecciona el uso",bano_vivienda:"Baño de vivienda",aseo_vivienda:"Aseo de vivienda",cocina_vivienda:"Cocina · ventilación general",campana_coccion:"Zona de cocción · extractor independiente",garaje:"Aparcamiento / garaje",trastero:"Trastero / zona común",residuos:"Almacén de residuos",rite:"Otro local · RITE"})[v]||v||"Selecciona el uso"}
function opcionesTipoNormativoZona(sel){return [["sin_clasificar","Selecciona el uso"],["bano_vivienda","Baño de vivienda"],["aseo_vivienda","Aseo de vivienda"],["cocina_vivienda","Cocina · ventilación general"],["campana_coccion","Zona de cocción · extractor independiente"],["garaje","Aparcamiento / garaje"],["trastero","Trastero / zona común"],["residuos","Almacén de residuos"],["rite","Otro local · RITE"]].map(([v,n])=>`<option value="${v}" ${String(sel||"")===v?"selected":""}>${n}</option>`).join("")}
function nombreSugeridoTipoNormativo(v){return ({bano_vivienda:"Baño",aseo_vivienda:"Aseo",cocina_vivienda:"Cocina",campana_coccion:"Zona de cocción",garaje:"Garaje",trastero:"Trastero",residuos:"Almacén de residuos",rite:"Local"})[v]||""}
...
function sistemasExtraccionCalculados(items,modo,sim,margen){
  ...
  (items||[]).forEach((item,i)=>{
    ...
    const clave=modo==="avanzado"?"manual_general":claveSistemaExtraccion(item,i),nombre=modo==="avanzado"?"Extracción manual":nombreSistemaExtraccion(clave,item);
    if(!mapa.has(clave))mapa.set(clave,{clave,nombre,items:[],independiente:modo!=="avanzado"&&clave!=="vivienda_general"});
    mapa.get(clave).items.push(item);
  });
  return [...mapa.values()].map(s=>{
    const base=s.items.reduce((a,x)=>a+nExt(x.calc.caudal_m3h),0),q=base*(sim/100)*(1+margen/100),presBase=s.items.reduce((a,x)=>Math.max(a,nExt(x.calc.perdida_pa)),0),p=presBase*(1+margen/100);
    return Object.assign(s,{caudal_base_m3h:redondearCalc(base,1),caudal_diseno_m3h:redondearCalc(q,1),presion_diseno_pa:redondearCalc(p,1),zonas:s.items.map(x=>Object.assign({},x.zona,x.calc,{normativa_ref:x.norm&&x.norm.ref||x.zona.normativa_ref||""}))});
  });
}
function valorIDA(v){return ({IDA1:20,IDA2:12.5,IDA3:8,IDA4:5})[v]||null}
...
function calcularNormativaZonas(zonas,config){
  ...
  return zonas.map((z,i)=>{
    const t=tipos[i],sup=Math.max(0,nExt(z.superficie_m2)),plazas=Math.max(0,nExt(z.plazas)),ocup=Math.max(0,nExt(z.ocupantes)),ida=String(z.ida||"IDA3");let ls=null,ref="",estado="ok",independiente=false;
    if(esLocalHumedoVivienda(t)){
      ...
      const vp=valorIDA(ida);if(ocup>0&&vp){ls=vp*ocup;ref=`RITE · IT 1.1.4.2.3 · método por persona · ${ida} · ${String(vp).replace('.',',')} l/s por persona`;estado="rite_revisar"}else{estado="falta_rite";ref="Indica ocupantes y categoría IDA. En usos con contaminantes específicos puede ser necesario otro método RITE."}
    }else{estado="sin_clasificar";ref="Selecciona el uso del local para determinar la regla aplicable"}
    return {tipo:t,caudal_ls:ls==null?null:redondearCalc(ls,2),caudal_m3h:ls==null?0:redondearCalc(ls*3.6,1),ref,estado,independiente};
  });
}
...
function calcZonaExtraccion(z,caudalForzado=null,diametroForzado=null){
  ...
  const guardado=Math.max(0,nExt(z.caudal_calculo_m3h)),caudal=caudalForzado!=null?Math.max(0,nExt(caudalForzado)):(guardado>0?guardado:(modo==="caudal"?manual:volumen*ren)),forma=z.forma_conducto||"sin_dimensionar";let area=0,dh=0;
...
function resumenExtraccion(p){
  const x=metaExtraccionProyecto(p),items=x.zonas.map((z,i)=>{const normGuardado=!!(z.normativa_ref&&nExt(z.caudal_calculo_m3h)>0),zz=normGuardado?Object.assign({},z,{presion_adicional_pa:0}):z,calc=normGuardado?calcZonaExtraccion(zz,z.caudal_calculo_m3h,z.diametro_recomendado_mm||null):calcZonaExtraccion(z),norm={tipo:tipoNormativoZona(z),independiente:!!z.independiente_normativa,ref:z.normativa_ref||""};return {zona:z,norm,dim:null,calc}}).filter(x=>x.calc.caudal_m3h>0),modo=x.modo_asistente||"normativa",sim=modo==="normativa"?100:Math.min(100,Math.max(0,nExt(x.simultaneidad_pct,100))),margen=Math.max(0,nExt(x.margen_pct,10)),sistemas=sistemasExtraccionCalculados(items,modo,sim,margen);
...
function calculosZonasFormulario(){
  const zs=zonasExtraccionFormulario(),modo=document.getElementById("pr_extract_mode")?.value||"normativa",norms=calcularNormativaZonas(zs,configNormativaFormulario());
  return zs.map((z,i)=>{
    if(modo==="avanzado")return {zona:z,norm:norms[i],dim:null,calc:calcZonaExtraccion(z)};
    const n=norms[i],dim=n.caudal_m3h>0&&["rite"].indexOf(n.tipo)<0?seccionCTEMecanica(n.caudal_m3h,z.ubicacion_conducto):null,diam=dim?dim.diametro_comercial_mm:null,calc=calcZonaExtraccion(Object.assign({},z,{presion_adicional_pa:0}),n.caudal_m3h,diam);
    return {zona:z,norm:n,dim,calc};
  });
}
function actualizarVisibilidadZonaExtraccion(card){
  const t=card.querySelector('[data-pr-extract="norma_tipo"]')?.value||"sin_clasificar",show=(grupo,on)=>{const e=card.querySelector(`[data-pr-extract-group="${grupo}"]`);if(e)e.hidden=!on};show("superficie",["trastero","residuos"].includes(t));show("plazas",t==="garaje");show("ocupantes",t==="rite");show("ida",t==="rite");
}
...
function resumenExtraccionFormulario(){
  pintarCalculosExtraccion();const todos=calculosZonasFormulario(),cs=todos.filter(x=>x.calc.caudal_m3h>0),modo=document.getElementById("pr_extract_mode")?.value||"normativa",sim=modo==="normativa"?100:Math.min(100,Math.max(0,nExt(document.getElementById("pr_extract_sim")?.value,100))),margen=modo==="normativa"?10:Math.max(0,nExt(document.getElementById("pr_extract_margin")?.value,10)),sistemas=sistemasExtraccionCalculados(cs,modo,sim,margen),ctx=contextoNormativoExtraccion(PROYECTO_EXTRACCION_ACTUAL||{});
  const simWrap=document.getElementById("pr_extract_sim_wrap");if(simWrap)simWrap.hidden=modo==="normativa";const marginWrap=document.getElementById("pr_extract_margin_wrap");if(marginWrap)marginWrap.hidden=modo==="normativa";const dormWrap=document.getElementById("pr_extract_dorm_wrap");if(dormWrap)dormWrap.hidden=modo==="avanzado";
  const modeTag=document.getElementById("pr_extract_mode_tag"),modeInfo=document.getElementById("pr_extract_mode_info");if(modeTag)modeTag.textContent=modo==="normativa"?"EXTRACCIÓN · ASISTENTE GUIADO":"EXTRACCIÓN · TÉCNICO";if(modeInfo){const b=modeInfo.querySelector("b"),sp=modeInfo.querySelector("span");if(b)b.textContent=modo==="normativa"?"Solo te pediremos lo que puedas observar o medir":"Control técnico directo";if(sp)sp.textContent=modo==="normativa"?"Elige el uso de cada espacio y describe el recorrido. Zentryx decide qué regla corresponde, calcula el mínimo aplicable y propone el conducto. Los parámetros de ingeniería quedan en Técnico.":"Puedes introducir caudal, renovaciones, dimensiones del conducto y presión adicional cuando dispongas de datos de proyecto, medición o fabricante."}
  renderSistemasExtraccionFormulario(sistemas);
  const status=document.getElementById("pr_extract_norm_status");if(status){const incompleta=modo==="normativa"&&(todos.length===0||todos.some(x=>["sin_clasificar","falta_dormitorios","falta_plazas","falta_superficie","falta_rite"].includes(x.norm.estado)));status.className="zx_pr_norm_box "+(incompleta?"is-bad":"is-warn");status.innerHTML=`<b>${incompleta?"Faltan datos para aplicar la norma":"Cálculo estatal aplicado"}</b><span>${ctx.esEspana?`CTE DB HS 3 / RITE según uso · ${limpiar(textoUbicacionNormativa(ctx))}. Reglas verificadas ${NORMATIVA_EXTRACCION_ES.verificado_el}.${ctx.esMadridCiudad?" Madrid ciudad: identificadas también la Ordenanza 4/2021 de Calidad del Aire y Sostenibilidad y la Guía municipal de ventilación versión enero 2026; las comprobaciones del punto de evacuación siguen pendientes de registrar.":" La revisión autonómica y municipal debe quedar registrada antes de cerrar el proyecto."}`:"No hay reglas normativas cargadas para "+limpiar(ctx.pais)+"."}</span>`}
  const general=sistemas.find(x=>x.clave==="vivienda_general")||sistemas[0]||null;
  return {simultaneidad_pct:sim,margen_pct:margen,caudal_base_m3h:general?general.caudal_base_m3h:0,caudal_diseno_m3h:general?general.caudal_diseno_m3h:0,presion_diseno_pa:general?general.presion_diseno_pa:0,sistemas,calculadas:cs,todas:todos,ctx,modo};
}
function conectarZonaExtraccion(card){
  const refresh=()=>{actualizarVisibilidadZonaExtraccion(card);resumenExtraccionFormulario()},nombre=card.querySelector('[data-pr-extract="nombre"]'),tipo=card.querySelector('[data-pr-extract="norma_tipo"]');
  card.dataset.autoName=nombre&&nombre.value.trim()?"0":"1";
  card.querySelectorAll("input,select").forEach(e=>{if(e===nombre||e===tipo)return;e.oninput=refresh;e.onchange=refresh});
...
async function guardarExtraccion(p){
  ...
  if(modo==="normativa"){
    ...
    const bad=todos.find(x=>["sin_clasificar","falta_dormitorios","falta_plazas","falta_superficie","falta_rite"].includes(x.norm.estado));if(bad){alert("Faltan datos para aplicar la normativa en una o más zonas. Revisa los avisos antes de guardar.");return}
  }
  if(!calc.length){alert("No hay ningún caudal calculado para guardar.");return}
  const zonas=calc.map(x=>Object.assign({},x.zona,{zona_id:x.zona.zona_id||uidExtraccion(),caudal_calculo_m3h:x.calc.caudal_m3h,velocidad_guardada_ms:x.calc.velocidad_ms,perdida_guardada_pa:x.calc.perdida_pa,norma_tipo:x.norm.tipo,normativa_ref:modo==="normativa"?x.norm.ref:null,caudal_normativo_m3h:modo==="normativa"?x.norm.caudal_m3h:null,seccion_min_cm2:modo==="normativa"&&x.dim?x.dim.seccion_cm2:null,diametro_recomendado_mm:modo==="normativa"&&x.dim?x.dim.diametro_comercial_mm:null,independiente_normativa:modo==="normativa"?x.norm.independiente:false}));
  const u=sesion(),meta=p.inmueble_meta&&typeof p.inmueble_meta==="object"&&!Array.isArray(p.inmueble_meta)?{...p.inmueble_meta}:{},prev=metaExtraccionProyecto(p),ahora=new Date().toISOString(),sistemas=r.sistemas.map(sys=>{
    ...
    return {clave:sys.clave,nombre:sys.nombre,independiente:!!sys.independiente,zonas_ids:sys.items.map(x=>x.zona.zona_id).filter(Boolean),caudal_base_m3h:sys.caudal_base_m3h,caudal_diseno_m3h:sys.caudal_diseno_m3h,presion_diseno_pa:sys.presion_diseno_pa,extractor_material_id:sel||null,extractor_snapshot:snapshot};
  }),general=sistemas.find(x=>x.clave==="vivienda_general")||sistemas[0]||null,normativa={estado:modo==="normativa"?"estatal_ok_territorial_pendiente":"manual",ruleset:NORMATIVA_EXTRACCION_ES.ruleset,verificado_el:NORMATIVA_EXTRACCION_ES.verificado_el,pais:r.ctx.pais,provincia:r.ctx.provincia,municipio:r.ctx.municipio,cte:NORMATIVA_EXTRACCION_ES.cte,rite:NORMATIVA_EXTRACCION_ES.rite,revision_territorial:r.ctx.esMadridCiudad?"parcial":"pendiente",municipal:r.ctx.esMadridCiudad?{norma:"Madrid · Ordenanza 4/2021 de Calidad del Aire y Sostenibilidad",guia:"Guía de comprobaciones de ventilación · versión enero 2026",estado:"punto de evacuación pendiente de comprobar"}:null};
  meta.extraccion={version:3,modo_asistente:modo,dormitorios_grupo:document.getElementById("pr_extract_dorm").value||"",simultaneidad_pct:r.simultaneidad_pct,margen_pct:r.margen_pct,descarga:document.getElementById("pr_extract_descarga").value.trim()||null,zonas,sistemas,caudal_base_m3h:general?general.caudal_base_m3h:0,caudal_diseno_m3h:general?general.caudal_diseno_m3h:0,presion_diseno_pa:general?general.presion_diseno_pa:0,extractor_material_id:general?general.extractor_material_id:null,extractor_snapshot:general?general.extractor_snapshot:null,normativa,notas:document.getElementById("pr_extract_notas").value.trim()||null,actualizado_at:ahora,actualizado_por:u.nombre||u.usuario||""};
