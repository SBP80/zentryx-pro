...
function especificacionesTecnicas(t){return Array.isArray(t&&t.especificaciones)?t.especificaciones.filter(x=>x&&typeof x==="object").map(x=>({nombre:String(x.nombre||"").trim(),valor:String(x.valor??"").trim(),medida:String(x.medida||x.unidad||"").trim()})).filter(x=>x.nombre||x.valor||x.medida):[]}
...
const DATOS_TECNICOS_SUGERIDOS={
  general:["Material","Medida","Acabado","Color","Marca","Modelo","Referencia","Norma","Compatibilidad","Peso","Dimensiones"],
  ...
  otro:["Tipo","Material","Medida","Capacidad","Potencia","Tensión nominal","Presión máxima","Temperatura máxima","Peso","Dimensiones","Norma","Compatibilidad"]
};
...
const MEDIDAS_TECNICAS_GENERALES=["mm","cm","m","mm²","cm²","m²","mm³","cm³","m³","L","ml","L/min","L/h","m³/h","W","kW","V","A","mA","Ω","Hz","bar","mbar","Pa","kPa","MPa","m.c.a.","°C","K","%","kg","g","Nm","rpm","dB(A)","W/mK","W/m²K","W/K","ud","h","min","s"];
...
function sugerenciasMedidaTecnica(nombre){const xs=MEDIDAS_TECNICAS_POR_DATO[claveDatoTecnico(nombre)]||[];return [...new Set(xs.concat(MEDIDAS_TECNICAS_GENERALES))]}
...
function filaEspecificacionHTML(x,categoria){x=x||{};const nombre=String(x.nombre||"").trim(),valor=String(x.valor??"").trim(),medida=String(x.medida||x.unidad||"").trim();const datos=sugerenciasDatoTecnico(categoria||"general"),valores=sugerenciasValorTecnico(nombre),medidas=sugerenciasMedidaTecnica(nombre);const nombreHit=opcionCoincidente(nombre,datos),valorHit=opcionCoincidente(valor,valores),medidaHit=opcionCoincidente(medida,medidas);return `<div class="zx_pr_cat_spec_row"><div class="zx_pr_cat_spec_field"><select data-pr-cat-spec-nombre-choice>${opcionesConPersonalizar(nombre,datos,"Seleccionar dato")}</select><input data-pr-cat-spec-nombre value="${limpiar(nombre)}" placeholder="Escribe el dato" ${nombre&& !nombreHit?"":"hidden"}></div><div class="zx_pr_cat_spec_field"><select data-pr-cat-spec-valor-choice>${opcionesConPersonalizar(valor,valores,"Seleccionar valor")}</select><input data-pr-cat-spec-valor value="${limpiar(valor)}" placeholder="Escribe el valor" ${valor&& !valorHit?"":"hidden"}></div><div class="zx_pr_cat_spec_field"><select data-pr-cat-spec-medida-choice>${opcionesConPersonalizar(medida,medidas,"Seleccionar medida")}</select><input data-pr-cat-spec-medida value="${limpiar(medida)}" placeholder="Escribe la medida" ${medida&& !medidaHit?"":"hidden"}></div><button type="button" data-pr-cat-spec-del aria-label="Quitar dato">×</button></div>`}
function sincronizarCampoEspec(row,tipo){const sel=row.querySelector(`[data-pr-cat-spec-${tipo}-choice]`),inp=row.querySelector(`[data-pr-cat-spec-${tipo}]`);if(!sel||!inp)return;const custom=sel.value==="__personalizar__";if(custom){inp.hidden=false;if(!inp.value)inp.focus()}else{inp.hidden=true;inp.value=sel.value||""}}
function reconstruirValorMedidaEspec(row){const nombre=row.querySelector("[data-pr-cat-spec-nombre]")?.value||"",valor=row.querySelector("[data-pr-cat-spec-valor]")?.value||"",medida=row.querySelector("[data-pr-cat-spec-medida]")?.value||"";const sv=row.querySelector("[data-pr-cat-spec-valor-choice]"),sm=row.querySelector("[data-pr-cat-spec-medida-choice]");if(sv)sv.innerHTML=opcionesConPersonalizar(valor,sugerenciasValorTecnico(nombre),"Seleccionar valor");if(sm)sm.innerHTML=opcionesConPersonalizar(medida,sugerenciasMedidaTecnica(nombre),"Seleccionar medida");sincronizarCampoEspec(row,"valor");sincronizarCampoEspec(row,"medida")}
function configurarFilaEspecificacion(row,categoria){if(!row)return;const nombre=row.querySelector("[data-pr-cat-spec-nombre]")?.value||"",sel=row.querySelector("[data-pr-cat-spec-nombre-choice]");if(sel)sel.innerHTML=opcionesConPersonalizar(nombre,sugerenciasDatoTecnico(categoria||"general"),"Seleccionar dato");sincronizarCampoEspec(row,"nombre");reconstruirValorMedidaEspec(row);const sn=row.querySelector("[data-pr-cat-spec-nombre-choice]"),sv=row.querySelector("[data-pr-cat-spec-valor-choice]"),sm=row.querySelector("[data-pr-cat-spec-medida-choice]");if(sn)sn.onchange=()=>{sincronizarCampoEspec(row,"nombre");reconstruirValorMedidaEspec(row)};if(sv)sv.onchange=()=>sincronizarCampoEspec(row,"valor");if(sm)sm.onchange=()=>sincronizarCampoEspec(row,"medida");const ni=row.querySelector("[data-pr-cat-spec-nombre]");if(ni)ni.onchange=()=>reconstruirValorMedidaEspec(row)}
