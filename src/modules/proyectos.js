...
function metaExtraccionProyecto(p){
  ...
  return Object.assign({version:2,modo_asistente:"normativa",dormitorios_grupo:"",simultaneidad_pct:100,margen_pct:10,zonas:[]},x,{zonas:Array.isArray(x.zonas)?x.zonas:[]});
}
...
function calcularNormativaZonas(zonas,config){
  ...
  return zonas.map((z,i)=>{
    ...
    if(esLocalHumedoVivienda(t)){
      ...
      else{ls=reparto;ref=`CTE DB HS 3 · tabla 2.1 · ${dorm.texto} · mínimo total húmedos ${dorm.total} l/s · mínimo por local ${dorm.local} l/s · reparto automático entre ${nHum} local(es) húmedo(s)`}
...
function resumenExtraccion(p){
  const x=metaExtraccionProyecto(p),zs=x.zonas.map(z=>{const norm=z.normativa_ref&&nExt(z.caudal_calculo_m3h)>0,zz=norm?Object.assign({},z,{presion_adicional_pa:0}):z,c=norm?calcZonaExtraccion(zz,z.caudal_calculo_m3h,z.diametro_recomendado_mm||null):calcZonaExtraccion(z);return Object.assign({},z,c)}).filter(z=>z.caudal_m3h>0),modo=x.modo_asistente||"normativa",sim=modo==="normativa"?100:Math.min(100,Math.max(0,nExt(x.simultaneidad_pct,100))),margen=Math.max(0,nExt(x.margen_pct,10)),base=zs.filter(z=>!z.independiente_normativa).reduce((s,z)=>s+z.caudal_m3h,0),q=base*(sim/100)*(1+margen/100),pres=zs.filter(z=>!z.independiente_normativa).reduce((m,z)=>Math.max(m,nExt(z.perdida_pa)),0)*(1+margen/100),snap=x.extractor_snapshot||null,datos=datosExtractorTecnico(snap),cumpleQ=datos.caudal_m3h==null?null:datos.caudal_m3h>=q,cumpleP=datos.presion_pa==null?null:datos.presion_pa>=pres;
...
function bloqueExtraccionFichaHTML(p){
  ...
  return `<section class="zx_pr_section zx_pr_extract_section"><div class="zx_pr_section_head"><div><h3>Extracción</h3><span>${r.zonas.length} zona(s) calculada(s)</span></div>${puedeEditar()?`<button id="pr_extract_edit" class="zx_pr_small_primary" type="button">${r.zonas.length?"Editar extracción":"Configurar extracción"}</button>`:""}</div>${estadoNormativoFichaHTML(x)}<div class="zx_pr_extract_summary"><div><span>Caudal de diseño</span><b>${r.caudal_diseno_m3h>0?limpiar(r.caudal_diseno_m3h)+" m³/h":"Sin calcular"}</b></div><div><span>Presión de diseño</span><b>${r.presion_diseno_pa>0?limpiar(r.presion_diseno_pa)+" Pa":"Sin calcular"}</b></div><div><span>Simultaneidad</span><b>${limpiar(x.simultaneidad_pct??100)} %</b></div><div><span>Margen</span><b>${limpiar(x.margen_pct??10)} %</b></div></div>${r.zonas.length?`<div class="zx_pr_extract_list">${r.zonas.map(z=>`<div><b>${limpiar(z.nombre||z.uso||"Zona")}</b><span>${limpiar(z.caudal_m3h)} m³/h${z.velocidad_ms!=null?" · "+limpiar(z.velocidad_ms)+" m/s":""}${z.perdida_pa>0?" · "+limpiar(z.perdida_pa)+" Pa":""}${z.normativa_ref?`</span><small>${limpiar(z.normativa_ref)}</small>`:"</span>"}</div>`).join("")}</div>`:`<div class="zx_pr_empty">Extracción está activa, pero todavía no tiene zonas calculadas.</div>`}<div class="zx_pr_extract_equipment"><span>Extractor seleccionado</span><b>${limpiar(nombre)}</b>${snap?estadoExtractorHTML(r):""}</div>${x.descarga?`<div class="zx_pr_calc_help zx_pr_spec_help">Descarga prevista: ${limpiar(x.descarga)}</div>`:""}</section>`;
}
...
function resumenExtraccionFormulario(){
  pintarCalculosExtraccion();const todos=calculosZonasFormulario(),cs=todos.filter(x=>x.calc.caudal_m3h>0),modo=document.getElementById("pr_extract_mode")?.value||"normativa",sim=modo==="normativa"?100:Math.min(100,Math.max(0,nExt(document.getElementById("pr_extract_sim")?.value,100))),margen=modo==="normativa"?10:Math.max(0,nExt(document.getElementById("pr_extract_margin")?.value,10)),base=cs.filter(x=>!x.norm.independiente).reduce((s,x)=>s+x.calc.caudal_m3h,0),q=base*(sim/100)*(1+margen/100),p=cs.filter(x=>!x.norm.independiente).reduce((m,x)=>Math.max(m,nExt(x.calc.perdida_pa)),0)*(1+margen/100),sel=document.getElementById("pr_extract_catalogo")?.value||"",mat=MATERIALES.find(m=>String(m.id)===String(sel)),datos=datosExtractorTecnico(mat),cq=datos.caudal_m3h==null?null:datos.caudal_m3h>=q,cp=datos.presion_pa==null?null:datos.presion_pa>=p,ctx=contextoNormativoExtraccion(PROYECTO_EXTRACCION_ACTUAL||{});
  ...
  const resume=document.getElementById("pr_extract_resume");if(resume)resume.innerHTML=`<div><span>Caudal base</span><b>${redondearCalc(base,1)} m³/h</b></div><div><span>Caudal de diseño</span><b>${redondearCalc(q,1)} m³/h</b></div><div><span>Presión de diseño</span><b>${redondearCalc(p,1)} Pa</b></div><div><span>Extractor</span><b>${mat?limpiar(materialTexto(mat)||mat.nombre):"Sin seleccionar"}</b></div>`;
  const info=document.getElementById("pr_extract_catalogo_info");if(info)info.innerHTML=mat?`<b>${limpiar(materialTexto(mat)||mat.nombre)}</b><span>${datos.caudal_m3h!=null?"Caudal catálogo: "+limpiar(datos.caudal_m3h)+" m³/h":"Caudal de catálogo sin indicar"}${datos.presion_pa!=null?" · Presión: "+limpiar(datos.presion_pa)+" Pa":" · Presión sin indicar"}</span><em class="${cq===false||cp===false?"is-bad":cq===true&&cp===true?"is-ok":""}">${cq===false||cp===false?"No alcanza el cálculo actual":cq===true&&cp===true?"Cumple caudal y presión calculados":"Faltan datos para comprobar"}</em>`:'Selecciona un artículo clasificado como Extracción en el Catálogo técnico para comparar caudal y presión.';
