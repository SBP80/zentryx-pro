...
function formularioExtraccion(p){
  PROYECTO_EXTRACCION_ACTUAL=p;const x=metaExtraccionProyecto(p),ctx=contextoNormativoExtraccion(p),modo=x.modo_asistente||"normativa";EXTRACTORES_EXTRACCION_FORM=seleccionesSistemasGuardadas(x);const m=modal(`<div class="zx_pr_top_actions"><button id="pr_extract_back" type="button">← Volver</button><button id="pr_extract_save" class="primary" type="button">Guardar extracción</button></div><div class="zx_pr_form_head"><span>EXTRACCIÓN · ASISTENTE NORMATIVO</span><h2>${limpiar(p.nombre)}</h2></div><div class="zx_pr_info"><b>Primero la norma, después el dimensionado</b><span>Zentryx propone el caudal mínimo según el uso y la ubicación del proyecto. Los campos técnicos manuales quedan separados para cuando dispongas de datos de proyecto o fabricante.</span></div><div class="zx_pr_section"><h3>Normativa y ubicación</h3><div class="zx_pr_norm_location"><span>Obra</span><b>${limpiar(ctx.direccion)}</b><small>${limpiar(textoUbicacionNormativa(ctx))}</small></div><label>Modo de trabajo<select id="pr_extract_mode"><option value="normativa" ${modo==="normativa"?"selected":""}>Asistente normativo</option><option value="avanzado" ${modo==="avanzado"?"selected":""}>Modo avanzado</option></select></label><label id="pr_extract_dorm_wrap">Dormitorios de la vivienda<select id="pr_extract_dorm"><option value="" ${!x.dormitorios_grupo?"selected":""}>Selecciona</option><option value="01" ${x.dormitorios_grupo==="01"?"selected":""}>0 o 1 dormitorio</option><option value="2" ${x.dormitorios_grupo==="2"?"selected":""}>2 dormitorios</option><option value="3m" ${x.dormitorios_grupo==="3m"?"selected":""}>3 o más dormitorios</option></select></label><div id="pr_extract_norm_status" class="zx_pr_norm_box"></div></div><div class="zx_pr_section"><h3>Criterios del sistema</h3><div class="zx_pr_calc_help">En el asistente normativo Zentryx mantiene el 100 % del caudal mínimo exigido. El margen solo se usa para escoger cada extractor, no para rebajar la exigencia reglamentaria.</div><div class="zx_pr_grid2"><label id="pr_extract_sim_wrap">Simultaneidad <span class="zx_pr_unit">%</span><input id="pr_extract_sim" type="number" min="0" max="100" step="1" inputmode="decimal" value="${limpiar(x.simultaneidad_pct??100)}"></label><label id="pr_extract_margin_wrap">Margen para seleccionar extractor <span class="zx_pr_unit">%</span><input id="pr_extract_margin" type="number" min="0" step="1" inputmode="decimal" value="${limpiar(x.margen_pct??10)}"></label></div><label>Punto de descarga / salida<input id="pr_extract_descarga" value="${limpiar(x.descarga||"")}" placeholder="Cubierta, fachada, conducto común…"></label></div><div class="zx_pr_section"><div class="zx_pr_section_head"><div><h3>Zonas de extracción</h3><span>Selecciona el uso y Zentryx aplica la regla correspondiente</span></div><button id="pr_extract_add" class="zx_pr_small_primary" type="button">＋ Añadir zona</button></div><div id="pr_extract_zones" class="zx_pr_extract_zones">${x.zonas.map(z=>zonaExtraccionHTML(z,modo)).join("")}</div></div><div class="zx_pr_section"><h3>Sistemas y extractores</h3><div class="zx_pr_calc_help">Zentryx separa los recorridos que requieren sistemas distintos. Cada sistema conserva su propio caudal de diseño, presión y extractor.</div><div id="pr_extract_systems" class="zx_pr_extract_systems"></div></div><label>Notas técnicas<textarea id="pr_extract_notas" rows="4" placeholder="Criterio empleado, condicionantes, revisión territorial, observaciones…">${limpiar(x.notas||"")}</textarea></label>`);
