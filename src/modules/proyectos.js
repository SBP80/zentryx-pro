...
function metaFontaneriaProyecto(p){
  ...
  return Object.assign({version:1,modo_asistente:"normativa",puntos:[],tecnico:{material_red:"termoplastica_multicapa",coef_sim_af_pct:null,coef_sim_acs_pct:null,velocidad_ms:1,presion_entrada_kpa:null,altura_geometrica_m:0,perdidas_kpa:0,punto_desfavorable:"grifo_comun"},notas:""},x,{puntos:Array.isArray(x.puntos)?x.puntos:[],tecnico:x.tecnico&&typeof x.tecnico==="object"&&!Array.isArray(x.tecnico)?x.tecnico:{}});
}
...
function pintarDatosPuntoFontaneria(row){
  ...
  box.innerHTML=`<span>AF <b>${limpiar(d.af)} L/s</b></span><span>ACS <b>${d.acs>0?limpiar(d.acs)+" L/s":"—"}</b></span><small>CTE DB HS 4 · tabla 2.1${ramal}</small>`;
}
...
function calculoTecnicoFontaneria(base,t){
  t=t&&typeof t==="object"?t:{};const kaf=numFontOpt(t.coef_sim_af_pct),kacs=numFontOpt(t.coef_sim_acs_pct),v=numFontOpt(t.velocidad_ms),material=t.material_red||"termoplastica_multicapa",rango=material==="metalica"?[0.5,2]:[0.5,3.5],qaf=kaf!=null?redondearCalc(base.af_ls*Math.max(0,Math.min(100,kaf))/100,3):null,qacs=kacs!=null?redondearCalc(base.acs_ls*Math.max(0,Math.min(100,kacs))/100,3):null,pin=numFontOpt(t.presion_entrada_kpa),h=Math.max(0,numFontOpt(t.altura_geometrica_m)||0),perd=Math.max(0,numFontOpt(t.perdidas_kpa)||0),req=t.punto_desfavorable==="fluxor_calentador"?150:100,pres=pin!=null?redondearCalc(pin-h*9.81-perd,1):null;
  return {coef_af_pct:kaf,coef_acs_pct:kacs,velocidad_ms:v,material_red:material,rango_vel:rango,q_af_ls:qaf,q_acs_ls:qacs,diam_af_mm:qaf!=null&&v?diametroTeoricoFontaneriaMm(qaf,v):null,diam_acs_mm:qacs!=null&&v?diametroTeoricoFontaneriaMm(qacs,v):null,presion_residual_kpa:pres,presion_requerida_kpa:req,presion_entrada_kpa:pin,velocidad_ok:v!=null&&v>=rango[0]&&v<=rango[1],presion_ok:pres!=null&&pres>=req&&pres<=500,presion_max_ok:pin==null||pin<=500};
}
function datosTecnicosFontaneriaFormulario(){
  return {material_red:document.getElementById("pr_water_material")?.value||"termoplastica_multicapa",coef_sim_af_pct:numModal("pr_water_kaf"),coef_sim_acs_pct:numModal("pr_water_kacs"),velocidad_ms:numModal("pr_water_vel"),presion_entrada_kpa:numModal("pr_water_pin"),altura_geometrica_m:numModal("pr_water_h")??0,perdidas_kpa:numModal("pr_water_loss")??0,punto_desfavorable:document.getElementById("pr_water_point")?.value||"grifo_comun"}
}
function resumenFontaneriaHTML(base,tec,modo){
  const filas=`<div><span>Puntos registrados</span><b>${base.unidades} ud</b></div><div><span>Caudal máximo AF</span><b>${limpiar(base.af_ls)} L/s</b></div><div><span>Caudal máximo ACS</span><b>${limpiar(base.acs_ls)} L/s</b></div>`;
  ...
  const qaf=tec.q_af_ls==null?"Pendiente":limpiar(tec.q_af_ls)+" L/s",qacs=tec.q_acs_ls==null?"Pendiente":limpiar(tec.q_acs_ls)+" L/s",daf=tec.diam_af_mm==null?"Pendiente":limpiar(tec.diam_af_mm)+" mm",dacs=tec.diam_acs_mm==null?"Pendiente":limpiar(tec.diam_acs_mm)+" mm",pr=tec.presion_residual_kpa==null?"Pendiente":limpiar(tec.presion_residual_kpa)+" kPa";
  let estado="Completa simultaneidad, velocidad y presión para comprobar la base de diseño.";let clase="";
  if(tec.coef_af_pct!=null&&tec.coef_acs_pct!=null&&tec.velocidad_ms!=null&&tec.presion_residual_kpa!=null){if(!tec.velocidad_ok){estado=`Revisar velocidad: para ${tec.material_red==="metalica"?"tubería metálica":"termoplástico/multicapa"} el intervalo CTE es ${tec.rango_vel[0]}–${tec.rango_vel[1]} m/s.`;clase="is-bad"}else if(!tec.presion_max_ok){estado="Revisar presión: la presión disponible no debe superar 500 kPa.";clase="is-bad"}else if(!tec.presion_ok){estado=`Presión insuficiente en el punto desfavorable: se requieren al menos ${tec.presion_requerida_kpa} kPa.`;clase="is-bad"}else{estado="Comprobación base correcta con los datos técnicos introducidos. Falta validar tramos, pérdidas reales y diámetros comerciales del proyecto.";clase="is-ok"}}
...
function actualizarResumenFontaneriaFormulario(){
  const modo=document.getElementById("pr_water_mode")?.value||"normativa",tech=document.getElementById("pr_water_tech"),tag=document.getElementById("pr_water_mode_tag"),info=document.getElementById("pr_water_mode_info");if(tech)tech.hidden=modo!=="avanzado";if(tag)tag.textContent=modo==="avanzado"?"FONTANERÍA · TÉCNICO":"FONTANERÍA · ASISTENTE GUIADO";if(info){info.querySelector("b").textContent=modo==="avanzado"?"Dimensionado técnico de la red":"Registra los puntos de consumo";info.querySelector("span").textContent=modo==="avanzado"?"Introduce el criterio de simultaneidad, material, velocidad y presión del proyecto. Zentryx calcula caudales de cálculo, diámetro interior teórico y una comprobación de presión.":"Selecciona los aparatos y cantidades. Zentryx aplica los caudales mínimos de CTE DB HS 4 y deja claramente pendiente el criterio de simultaneidad y el dimensionado de tramos."}
  const puntos=leerPuntosFontaneriaFormulario(),base=resumenFontaneriaDesdePuntos(puntos),tec=calculoTecnicoFontaneria(base,datosTecnicosFontaneriaFormulario()),box=document.getElementById("pr_water_results");if(box)box.innerHTML=resumenFontaneriaHTML(base,tec,modo)
}
...
function bloqueFontaneriaFichaHTML(p){
  if(!especialidadesProyecto(p).includes("fontaneria"))return "";const x=metaFontaneriaProyecto(p),base=resumenFontaneriaDesdePuntos(x.puntos),tec=calculoTecnicoFontaneria(base,x.tecnico),hay=base.unidades>0,modo=x.modo_asistente==="avanzado"?"Técnico":"Asistente guiado";
