...
function aplicarModoCalculo(){
  ...
  const textos={
    ...
    tecnico:["Registrar cálculo técnico","Introduce las condiciones y los resultados obtenidos con el método o programa técnico que utilices."],
  ...
  const auto=tipo==="estimacion";["pr_calc_cal","pr_calc_ref","pr_calc_acs","pr_calc_imp"].forEach(id=>{const el=document.getElementById(id);if(el){el.readOnly=auto;el.classList.toggle("zx_pr_calc_readonly",auto)}});
  if(help)help.textContent=auto?"Se actualizan al cambiar superficie, coeficientes, ocupantes o impulsión.":"Introduce los valores que quieras guardar en esta versión.";
  if(auto)actualizarEstimacionRapida();else limpiarResultadosAuto();
}
...
async function guardarCalculo(p,version){
  ...
  if(tipo==="estimacion"){
    ...
    entrada.temp_ext_invierno_c=numCalcSigned("pr_calc_te_inv","pr_calc_te_inv_sign");entrada.temp_int_calefaccion_c=numCalc("pr_calc_ti_cal");entrada.temp_ext_verano_c=numCalcSigned("pr_calc_te_ver","pr_calc_te_ver_sign");entrada.temp_int_refrigeracion_c=numCalc("pr_calc_ti_ref");
  }else if(tipo==="otro")entrada.metodo_nombre=document.getElementById("pr_calc_metodo").value.trim()||null;
  const resultado={creado_por_nombre:u.nombre||u.usuario||""};if(tipo==="estimacion"){resultado.automatico=true;resultado.formula_version=1}
