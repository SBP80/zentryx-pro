...
// V3115 - CARGA SEGURA + GESTIÓN SIMPLE
// ===============================
(function(){
"use strict";
...
async function cargarTodosTrabajos(){
  const r=await sb().from("trabajos").select("*");
  if(r.error) return [];
  return r.data || [];
}
...
function filtrarTrabajos(){
  ...
  if(b) datos=datos.filter(t=>textoBusquedaTrabajo(t).includes(normalizarTexto(b)));
  return datos;
}
...
async function cargarArchivos(trabajoId){
  ...
  return (r.data || []).filter(a=>{
    ...
    if(["factura","albaran","albarán","presupuesto","economico","económico","coste"].includes(tipo)) return puedeVerPrecios();
    return true;
  });
}
...
async function registrarAuditoriaTrabajo(trabajo,accion,datosExtra={}){
  const s=sesion();
  try{
    await sb().from("trabajos_auditoria").insert([{
...
async function pedirPinAdmin(){
  return new Promise(function(resolve){
    ...
    document.getElementById("tr_pin_cancelar").onclick=function(){cerrarModalTrabajo(); resolve(false)};
    document.getElementById("tr_pin_ok").onclick=async function(){
      ...
      if(!admin){alert("Solo administrador."); return}
...
function renderPlanificacion(lista){
  if(!lista || !lista.length) return `<div class="zx_text">Sin planificación asignada.</div>`;
  return `
    <div class="zx_plan_compact">
      ${lista.map(p=>`
        <div class="zx_plan_row">
          <div class="zx_plan_fecha">${limpiar(formatoFechaES(p.fecha || ""))}</div>
          <div class="zx_plan_hora">${limpiar(p.hora_inicio ? String(p.hora_inicio).slice(0,5) : "--:--")} - ${limpiar(p.hora_fin ? String(p.hora_fin).slice(0,5) : "--:--")}</div>
...
function renderArchivos(lista){
  if(!lista || !lista.length) return `<div class="zx_text">Sin archivos visibles.</div>`;
  return lista.map(a=>`
    <div class="zx_tr_file_item">
      <div><b>${limpiar(a.nombre || "Archivo")}</b><br><span>${limpiar(a.tipo || "-")}</span></div>
      <button class="zx_action_btn zx_blue" data-tr-open-file="${limpiar(a.url || "")}">Abrir</button>
    </div>
  `).join("");
}

function renderMateriales(lista){
  if(!lista || !lista.length) return `<div class="zx_text">Sin materiales.</div>`;
  return `
      ...
      ${lista.map(m=>`
        <div class="zx_tr_mat_item ${materialPreparado(m) ? "zx_tr_mat_preparado" : ""}">
          <label class="zx_admin_mat_check">
            <input type="checkbox" ${materialPreparado(m) ? "checked" : ""} data-mat-preparado="${limpiar(m.id)}">
            <span>
...
function renderHistorial(lista){
  if(!lista || !lista.length) return `<div class="zx_text">Sin historial.</div>`;
  return lista.map(h=>`
    <div class="zx_tr_hist_item">
      ...
      ${h.hora_fin ? " - "+limpiar(String(h.hora_fin).slice(0,5)) : ""}<br>
      Tipo: ${limpiar(h.tipo || "-")}
...
function renderMaterialesPreparacionObra(trabajoId,lista){
    ...
    return `
      <div class="zx_obra_material_preparacion zx_mat_estado_vacio">
        <div class="zx_mat_prepa_head">
          <div><b>Sin material pendiente</b><span>No hay lista de preparación para esta obra.</span></div>
  ...
  return `
    <div class="zx_obra_material_preparacion ${clasePreparacionMateriales(lista)}">
      <div class="zx_mat_prepa_head">
        <div><b>${r.pendientes===0 ? "Material preparado" : "Material pendiente"}</b><span>${limpiar(textoPreparacionMateriales(lista))}</span></div>
      ...
      <div class="zx_mat_prepa_lista">
        ${lista.map(m=>`
          <label class="zx_mat_prepa_item ${materialPreparado(m) ? "zx_mat_prepa_item_ok" : ""}">
            <input type="checkbox" ${materialPreparado(m) ? "checked" : ""} data-mat-preparado="${limpiar(m.id)}">
            <span><b>${limpiar(m.material || "Material")}</b><em>${limpiar(m.cantidad || "0")} ${limpiar(m.unidad || "")}</em>${m.notas ? `<small>${limpiar(m.notas)}</small>` : ""}</span>
          </label>
        `).join("")}
      </div>
    </div>
...
async function renderTrabajoObra(t){
  ...
  const horaTexto=primerPlan ? String(primerPlan.hora_inicio || "").slice(0,5)+" - "+String(primerPlan.hora_fin || "").slice(0,5) : "";
  ...
  return `
    <div class="zx_obra_card zx_obra_compacta ${claseTarjeta(t)}">
      <div class="zx_obra_top_compacta">
        ...
        <div class="zx_obra_estado_material ${clasePreparacionMateriales(materiales)}"><b>${mat.total ? mat.preparados+"/"+mat.total : "0"}</b><span>Material</span></div>
      </div>
      ...
      <details class="zx_obra_section" open>
        ...
        <div class="zx_obra_datos_compactos">
          ...
          <div><b>Prioridad</b><span>${limpiar(textoPrioridad(t.prioridad || "media"))}</span></div>
        </div>
      </details>

      <details class="zx_obra_section"><summary>Planificación</summary>${renderPlanificacion(plan)}</details>
      <details class="zx_obra_section" data-obra-section="planos" ${grupos.planos.length ? "open" : ""}><summary>Planos</summary>${renderBotonesArchivoObra("Planos",grupos.planos,"📄") || `<div class="zx_text">Sin planos.</div>`}</details>
      <details class="zx_obra_section" data-obra-section="fotos"><summary>Fotos</summary>${renderBotonesArchivoObra("Fotos",grupos.fotos,"📷") || `<div class="zx_text">Sin fotos.</div>`}</details>
      <details class="zx_obra_section" data-obra-section="videos"><summary>Vídeos</summary>${renderBotonesArchivoObra("Vídeos",grupos.videos,"🎥") || `<div class="zx_text">Sin vídeos.</div>`}</details>
      <details class="zx_obra_section"><summary>Manuales y otros documentos</summary>${renderBotonesArchivoObra("Manuales",grupos.manuales,"📘")}${renderBotonesArchivoObra("Otros",grupos.otros,"📎") || (!grupos.manuales.length ? `<div class="zx_text">Sin documentos.</div>` : "")}</details>
    </div>
  `;
}

async function renderTrabajo(t){
  if(!puedeGestionarTrabajos()) return await renderTrabajoObra(t);
  ...
  return `
    <div class="zx_user_card zx_tr_card ${claseTarjeta(t)}">
      ...
      <div class="zx_user_data">
        ...
        <b>Dirección obra:</b> ${dir ? `<button class="zx_link_dir" data-tr-map="${limpiar(dir)}">${limpiar(dir)}</button>` : "-"}<br>
        <b>Descripción:</b> ${limpiar(t.descripcion || "-")}<br>
        <b>Notas técnicas:</b> ${limpiar(t.notas || "-")}
      </div>

      <div class="zx_tr_panel"><h3>Planificación</h3>${renderPlanificacion(plan)}</div>
      <div class="zx_tr_panel"><h3>Archivos de obra</h3>${renderArchivos(archivos)}${puedeSubirArchivoTrabajo() ? `<button class="zx_btn_big zx_azul" data-tr-add-file="${limpiar(t.id)}">Añadir archivo</button>` : ""}</div>
      <div class="zx_tr_panel"><h3>Materiales</h3>${renderMateriales(materiales)}${puedeModificarMateriales() ? `<button class="zx_btn_big zx_azul" data-tr-add-material="${limpiar(t.id)}">Añadir material</button>` : ""}</div>
      <div class="zx_tr_panel"><h3>Historial</h3>${renderHistorial(historial)}${puedeModificarHistorial() ? `<button class="zx_btn_big zx_azul" data-tr-add-historial="${limpiar(t.id)}">Añadir historial</button>` : ""}</div>
  ...
  return `
    <div class="zx_tr_plan_form zx_plan_form_compact" data-plan-row="${i}">
      <div class="zx_plan_form_grid">
        <div><label class="zx_label">Fecha</label><input type="date" class="tr_plan_fecha" value="${limpiar(normalizarFecha(p.fecha || hoy()))}"></div>
        <div><label class="zx_label">Inicio</label><input type="time" class="tr_plan_inicio" value="${limpiar(p.hora_inicio ? String(p.hora_inicio).slice(0,5) : "")}"></div>
        <div><label class="zx_label">Fin</label><input type="time" class="tr_plan_fin" value="${limpiar(p.hora_fin ? String(p.hora_fin).slice(0,5) : "")}"></div>
      </div>
      <label class="zx_label">Operario</label>
      <select class="tr_plan_usuario">
        <option value="">Seleccionar operario</option>
        ${usuarios.map(u=>`
          <option value="${limpiar(u.id)}" data-usuario="${limpiar(u.usuario || "")}" data-nombre="${limpiar(u.nombre || u.usuario || "")}" ${String(p.usuario_id||"")===String(u.id) ? "selected" : ""}>${limpiar(u.nombre || u.usuario || "")}</option>
        `).join("")}
      </select>
      <details class="zx_plan_details"><summary>Notas de esta línea</summary><textarea class="tr_plan_notas" rows="2" placeholder="Notas para este día/operario">${limpiar(p.notas || "")}</textarea></details>
      <div class="zx_plan_buttons"><button class="zx_action_btn zx_blue tr_plan_duplicar" type="button">Duplicar</button><button class="zx_action_btn zx_red tr_plan_borrar" type="button">Quitar</button></div>
    </div>
  `;
}
...
async function comprobarSolapesPlanificacion(planificacion,trabajoIdActual=null){
  ...
  for(let i=0;i<filas.length;i++){
    ...
    if(minHora(p.hora_fin)<=minHora(p.hora_inicio)){alert("La hora fin debe ser posterior a la hora inicio."); return false}
    ...
    if(interno){
      alert("El operario está repetido en la planificación del mismo trabajo:\n\n"+(p.nombre || p.usuario || "Operario")+"\n"+formatoFechaES(p.fecha)+" · "+p.hora_inicio+" - "+p.hora_fin);
      return false;
    }

    const r=await sb().from("agenda_eventos").select("*").lte("fecha_inicio",p.fecha).gte("fecha_fin",p.fecha).neq("estado","cancelado");
    if(r.error){alert("Error comprobando solapes: "+r.error.message); return false}

    const solape=(r.data || []).find(e=>{
      if(String(e.origen)==="trabajos" && String(e.origen_id)===String(trabajoIdActual || "")) return false;
      const mismoId=String(e.usuario_id || "") && String(e.usuario_id || "")===String(p.usuario_id || "");
      const mismoNombre=String(e.usuario || "") && String(e.usuario || "").toLowerCase()===String(p.nombre || p.usuario || "").toLowerCase();
      if(!mismoId && !mismoNombre) return false;
      return rangosSolapan(p.hora_inicio,p.hora_fin,e.hora_inicio,e.hora_fin);
    });

    if(solape){
      alert("El operario ya tiene otro evento en ese horario:\n\n"+(p.nombre || p.usuario || "Operario")+"\n"+formatoFechaES(p.fecha)+" · "+p.hora_inicio+" - "+p.hora_fin+"\n\nEvento existente: "+(solape.titulo || "Evento"));
      return false;
    }
  }
...
async function formulario(t={}){
  if(t.id && !puedeEditarTrabajo()){alert("No tienes permiso para editar trabajos."); return}
  if(!t.id && !puedeCrearTrabajo()){alert("No tienes permiso para crear trabajos."); return}
    ...
    <div id="zx_modal_trabajo" class="zx_modal_fondo">
      <div class="zx_modal_caja">
        ...
        <select id="tr_cliente">
          ...
          ${clientes.map(c=>`<option value="${limpiar(c.id)}" ${String(t.cliente_id||"")===String(c.id) ? "selected" : ""}>${limpiar(c.nombre || "")}</option>`).join("")}
        </select>
        ...
        <button class="zx_btn_big zx_azul" id="tr_copiar_direccion" type="button">Usar dirección del cliente como obra</button>

        <h3 class="zx_form_subtitle">Planificación</h3>
        <div class="zx_text">Añade días, horarios y operarios.</div>
        <div id="tr_plan_lista">${planExistente.length ? planExistente.map((p,i)=>filaPlanificacionHTML(i,usuarios,p)).join("") : filaPlanificacionHTML(0,usuarios,{})}</div>
        <button class="zx_btn_big zx_azul" id="tr_add_plan" type="button">Añadir día / operario</button>
  ...
  if(t.cliente_id){
    ...
    if(c && !t.direccion_cliente) document.getElementById("tr_direccion_cliente").value=direccionCliente(c);
  }
...
async function subirArchivoObra(file,tipo,trabajoId){
  if(!puedeSubirArchivoTrabajo()){alert("No tienes permiso para subir archivos."); return null}
  ...
  const r=await sb().storage.from("zentryx-trabajos").upload(path,file,{upsert:true});
  if(r.error){alert("Error subiendo archivo: "+r.error.message); return null}
  return sb().storage.from("zentryx-trabajos").getPublicUrl(path).data.publicUrl;
}

window.ZX_tr_add_file=async function(trabajoId){
  if(!puedeSubirArchivoTrabajo()){alert("No tienes permiso para añadir archivos."); return}
  cerrarModalTrabajo();
  document.body.insertAdjacentHTML("beforeend",`
    <div id="zx_modal_trabajo" class="zx_modal_fondo">
      <div class="zx_modal_caja">
        ...
        <select id="tr_file_tipo"><option value="plano">Plano</option><option value="foto">Foto</option><option value="video">Vídeo</option><option value="documento">Documento técnico</option><option value="manual">Manual</option>${puedeVerPrecios() ? `<option value="factura">Factura / albarán</option>` : ""}<option value="otro">Otro</option></select>
        <label class="zx_label">Archivo</label><input id="tr_file_archivo" type="file">
        <label class="zx_label">Nombre visible</label><input id="tr_file_nombre" placeholder="Nombre del archivo">
...
function normalizarMaterialClave(v){
  return normalizarTexto(v).replace(/ø/g,"diametro").replace(/⌀/g,"diametro").replace(/\bdiam\b/g,"diametro").replace(/\s+/g," ").trim();
}
...
async function guardarMaterialEnCatalogo(nombre,unidad){
  ...
  try{
    ...
    const buscado=await sb().from("materiales_catalogo").select("id,veces_usado").eq("clave",clave).limit(1);
    if(buscado.error) return true;
    if(buscado.data && buscado.data.length){
      await sb().from("materiales_catalogo").update({nombre:n,unidad:u,veces_usado:Number(buscado.data[0].veces_usado || 0)+1,updated_at:new Date().toISOString()}).eq("id",buscado.data[0].id);
    }else{
      await sb().from("materiales_catalogo").insert([{nombre:n,clave,unidad:u,veces_usado:1,created_at:new Date().toISOString(),updated_at:new Date().toISOString()}]);
    }
  }catch(e){}
  return true;
}
...
async function guardarMaterialSumando(trabajoId,material,cantidad,unidad,notas){
  ...
  if(!nombre){alert("Introduce material."); return false}
  if(!Number.isFinite(cant) || cant<=0){alert("Introduce cantidad correcta."); return false}

  const existentes=await sb().from("trabajos_materiales").select("*").eq("trabajo_id",String(trabajoId));
  if(existentes.error){alert("Error buscando materiales: "+existentes.error.message); return false}

  const claveNombre=normalizarMaterialClave(nombre);
  const claveUnidad=normalizarMaterialClave(ud);
  const repetido=(existentes.data || []).find(m=>normalizarMaterialClave(m.material || "")===claveNombre && normalizarMaterialClave(m.unidad || "")===claveUnidad);

  if(repetido){
    const nuevaCantidad=Number(repetido.cantidad || 0)+cant;
    const notasFinales=nota ? [repetido.notas || "",nota].filter(Boolean).join(" / ") : (repetido.notas || "");
    const r=await sb().from("trabajos_materiales").update({cantidad:nuevaCantidad,unidad:ud,notas:notasFinales}).eq("id",repetido.id);
    if(r.error){alert("Error actualizando material: "+r.error.message); return false}
    await guardarMaterialEnCatalogo(nombre,ud);
    return true;
  }

  const r=await sb().from("trabajos_materiales").insert([{trabajo_id:String(trabajoId),material:nombre,cantidad:cant,unidad:ud,notas:nota,created_at:new Date().toISOString()}]);
  if(r.error){alert("Error guardando material: "+r.error.message); return false}
  await guardarMaterialEnCatalogo(nombre,ud);
  return true;
}

async function obtenerTrabajoYMateriales(trabajoId){
  const r=await sb().from("trabajos").select("*").eq("id",String(trabajoId)).maybeSingle();
  if(r.error || !r.data){alert("Trabajo no encontrado."); return null}
  const materiales=await cargarMateriales(trabajoId);
  return {trabajo:r.data,materiales:materiales || []};
}

function renderListaMaterialesModal(trabajo,materiales){
  return `
    <div class="zx_obra_material_modal_head"><b>${limpiar(trabajo.titulo || "Trabajo")}</b><span>${limpiar(trabajo.cliente || "")}</span></div>
    <div class="zx_obra_material_actions"><button class="zx_action_btn zx_blue" id="tr_mat_modal_imprimir">Imprimir</button><button class="zx_action_btn zx_blue" id="tr_mat_modal_enviar">Enviar</button></div>
    ${materiales.length ? `
      <div class="zx_mat_lista_compacta">
        ${materiales.map(m=>`
          <div class="zx_mat_linea_compacta ${materialPreparado(m) ? "zx_mat_linea_ok" : ""}" data-mat-row="${limpiar(m.id)}">
            <input type="checkbox" ${materialPreparado(m) ? "checked" : ""} data-mat-preparado="${limpiar(m.id)}">
            <button type="button" class="zx_mat_linea_texto" data-mat-select="${limpiar(m.id)}"><b>${limpiar(m.material || "Material")}</b><em>${limpiar(m.cantidad || "0")} ${limpiar(m.unidad || "")}</em>${m.notas ? `<small>${limpiar(m.notas)}</small>` : ""}</button>
          </div>
        `).join("")}
      </div>
      <div id="tr_mat_acciones_seleccion" class="zx_mat_acciones_seleccion zx_mat_acciones_ocultas">
        <div><b id="tr_mat_sel_nombre">Material seleccionado</b><span>Editar o borrar solo el material seleccionado.</span></div>
        <div class="zx_mat_acciones_botones"><button class="zx_action_btn zx_blue" id="tr_mat_sel_editar">Editar</button><button class="zx_action_btn zx_red" id="tr_mat_sel_borrar">Borrar</button></div>
      </div>
...
function seleccionarMaterialLista(materialId,materiales){
  document.querySelectorAll(".zx_mat_linea_compacta").forEach(x=>x.classList.toggle("zx_mat_linea_selected",String(x.dataset.matRow)===String(materialId)));
  ...
  if(nombre) nombre.textContent=(m.material || "Material")+" · "+(m.cantidad || "0")+" "+(m.unidad || "");
  if(editar) editar.onclick=function(){editarMaterialObra(m.id,m)};
  if(borrar) borrar.onclick=function(){borrarMaterialObra(m.id)};
}
...
async function editarMaterialObra(materialId,materialActual){
  ...
  if(!nombre){alert("El material no puede estar vacío."); return}
  ...
  const r=await sb().from("trabajos_materiales").update({material:nombre,cantidad,unidad,notas:String(nuevasNotas || "").trim()}).eq("id",String(materialId));
  if(r.error){alert("Error editando material: "+r.error.message); return}
...
async function borrarMaterialObra(materialId){
  ...
  if(r.error){alert("Error borrando material: "+r.error.message); return}
  const modal=document.getElementById("zx_modal_trabajo");
  const trabajoId=modal ? modal.dataset.trabajoId : "";
  if(trabajoId) abrirMaterialesObra(trabajoId,"lista");
  else ZX_trabajos();
...
async function cambiarPreparadoMaterial(materialId,preparado,refrescar=true){
  const r=await sb().from("trabajos_materiales").update({preparado,preparado_at:preparado ? new Date().toISOString() : null,preparado_por:preparado ? (sesion().usuario || "") : ""}).eq("id",String(materialId));
  if(r.error){alert("Error actualizando material preparado: "+r.error.message); return}
  if(refrescar) ZX_trabajos();
}
...
async function abrirMaterialesObra(trabajoId,modo){
  ...
  document.body.insertAdjacentHTML("beforeend",`
    <div id="zx_modal_trabajo" class="zx_modal_fondo" data-trabajo-id="${limpiar(trabajoId)}">
      <div class="zx_modal_caja">
        <h2>Materiales</h2>
        <div class="zx_obra_material_tabs"><button class="${!modoLista ? "zx_mat_tab_on" : ""}" id="tr_mat_tab_anotar">Anotar material</button><button class="${modoLista ? "zx_mat_tab_on" : ""}" id="tr_mat_tab_lista">Ver lista</button></div>
  ...
  async function pintarAnotar(){
    ...
    document.getElementById("tr_mat_modal_contenido").innerHTML=`
      <div class="zx_obra_material_modal_head"><b>${limpiar(datos.trabajo.titulo || "Trabajo")}</b><span>${limpiar(datos.trabajo.cliente || "")}</span></div>
      <label class="zx_label">Material</label><input id="tr_mat_obra_nombre" autocomplete="off" placeholder="Ej: codo de grimpar de 32">
      <div class="zx_tr_grid2"><div><label class="zx_label">Cantidad</label><input id="tr_mat_obra_cantidad" type="number" step="0.01" inputmode="decimal" value="1"></div><div><label class="zx_label">Unidad</label><input id="tr_mat_obra_unidad" value="ud" placeholder="ud, m, kg..."></div></div>
      <label class="zx_label">Notas</label><textarea id="tr_mat_obra_notas" rows="3" placeholder="Opcional"></textarea>
      <button class="zx_btn_big zx_verde" id="tr_mat_obra_guardar">Añadir a la lista</button>
      <div class="zx_permiso_info">Si el material ya existe con la misma unidad, se suma la cantidad automáticamente.</div>
    `;
    document.getElementById("tr_mat_obra_guardar").onclick=async function(){
      const ok=await guardarMaterialSumando(trabajoId,document.getElementById("tr_mat_obra_nombre").value,document.getElementById("tr_mat_obra_cantidad").value,document.getElementById("tr_mat_obra_unidad").value,document.getElementById("tr_mat_obra_notas").value);
      if(!ok) return;
  ...
  async function pintarLista(){
    ...
    document.getElementById("tr_mat_modal_imprimir").onclick=function(){ZX_tr_print_materiales(trabajoId)};
    document.getElementById("tr_mat_modal_enviar").onclick=function(){ZX_tr_share_materiales(trabajoId)};
    cont.querySelectorAll("[data-mat-preparado]").forEach(chk=>{
      chk.onchange=async function(e){e.stopPropagation(); await cambiarPreparadoMaterial(chk.dataset.matPreparado,chk.checked,false); await pintarLista()};
    });
    cont.querySelectorAll("[data-mat-select]").forEach(btn=>{
      btn.onclick=function(e){e.preventDefault(); e.stopPropagation(); seleccionarMaterialLista(btn.dataset.matSelect,nuevos.materiales)};
    });
    cont.querySelectorAll("[data-mat-row]").forEach(row=>{
      row.onclick=function(e){if(e.target && e.target.matches("input[type='checkbox']")) return; seleccionarMaterialLista(row.dataset.matRow,nuevos.materiales)};
    });
  }
  ...
  document.getElementById("tr_mat_modal_cerrar").onclick=function(){cerrarModalTrabajo(); ZX_trabajos()};
  if(modoLista) await pintarLista(); else await pintarAnotar();
}

window.ZX_tr_obra_materiales=async function(trabajoId){await abrirMaterialesObra(trabajoId,"anotar")};
window.ZX_tr_add_material=async function(trabajoId){
  if(!puedeModificarMateriales()){alert("No tienes permiso para añadir materiales."); return}
  await abrirMaterialesObra(trabajoId,"anotar");
};

window.ZX_tr_add_historial=async function(trabajoId){
  if(!puedeModificarHistorial()){alert("No tienes permiso para añadir historial."); return}
  const usuarios=await cargarUsuarios();
  cerrarModalTrabajo();
  document.body.insertAdjacentHTML("beforeend",`
    <div id="zx_modal_trabajo" class="zx_modal_fondo">
      <div class="zx_modal_caja">
        ...
        <select id="tr_hist_usuario"><option value="">Seleccionar usuario</option>${usuarios.map(u=>`<option value="${limpiar(u.id)}" data-usuario="${limpiar(u.usuario || "")}" data-nombre="${limpiar(u.nombre || u.usuario || "")}">${limpiar(u.nombre || u.usuario || "")}</option>`).join("")}</select>
        <label class="zx_label">Fecha</label><input id="tr_hist_fecha" type="date" value="${hoy()}">
        <div class="zx_tr_grid2"><div><label class="zx_label">Hora inicio</label><input id="tr_hist_inicio" type="time"></div><div><label class="zx_label">Hora fin</label><input id="tr_hist_fin" type="time"></div></div>
        ...
        <label class="zx_label">Notas</label><textarea id="tr_hist_notas" rows="3" placeholder="Notas"></textarea>
  ...
  document.getElementById("tr_hist_guardar").onclick=async function(){
    ...
    const r=await sb().from("trabajos_historial").insert([{trabajo_id:String(trabajoId),usuario_id:String(sel.value),usuario:opt ? opt.dataset.nombre || opt.dataset.usuario || "" : "",fecha:normalizarFecha(document.getElementById("tr_hist_fecha").value || hoy()),hora_inicio:document.getElementById("tr_hist_inicio").value || null,hora_fin:document.getElementById("tr_hist_fin").value || null,tipo:document.getElementById("tr_hist_tipo").value,notas:document.getElementById("tr_hist_notas").value.trim()}]);
...
async function sincronizarAgenda(trabajoId,data,planificacion){
  ...
  for(const p of planificacion){
    await sb().from("agenda_eventos").insert([{tipo:"trabajo",titulo:"Trabajo - "+data.titulo,descripcion:data.descripcion || "",fecha_inicio:normalizarFecha(p.fecha),fecha_fin:normalizarFecha(p.fecha),hora_inicio:p.hora_inicio,hora_fin:p.hora_fin,cliente_id:String(data.cliente_id || ""),cliente:data.cliente || "",usuario_id:String(p.usuario_id || ""),usuario:p.nombre || p.usuario || "",estado:data.estado==="terminado" ? "completado" : "activo",prioridad:data.prioridad || "media",visible_para:"todos",origen:"trabajos",origen_id:String(trabajoId),creado_por:data.creado_por || ""}]);
  }
}
...
async function guardarPlanificacion(trabajoId,lista){
  ...
  const insert=lista.map(p=>({trabajo_id:String(trabajoId),fecha:normalizarFecha(p.fecha),hora_inicio:p.hora_inicio,hora_fin:p.hora_fin,usuario_id:String(p.usuario_id || ""),usuario:p.usuario || "",nombre:p.nombre || "",notas:p.notas || ""}));
  const r=await sb().from("trabajos_planificacion").insert(insert);
  if(r.error) alert("Error guardando planificación: "+r.error.message);
}

async function guardarTrabajo(id,clientes){
  if(id && !puedeEditarTrabajo()){alert("No tienes permiso para editar trabajos."); return}
  if(!id && !puedeCrearTrabajo()){alert("No tienes permiso para crear trabajos."); return}
  const s=sesion();
  ...
  if(!planificacion.length){alert("Añade al menos un día, horario y operario."); return}
  ...
  if(id){
    ...
    r=await sb().from("trabajos").update(data).eq("id",id).select().maybeSingle();
  }else{
  ...
  await sincronizarAgenda(trabajoId,{...data,estado:id ? (r.data?.estado || "pendiente") : data.estado},planificacion);
  await registrarAuditoriaTrabajo(r.data || {id:trabajoId,titulo:data.titulo},id ? "EDITAR_TRABAJO" : "CREAR_TRABAJO",{cliente:data.cliente || "",prioridad:data.prioridad || "media"});
  cerrarModalTrabajo();
  ZX_trabajos();
}

window.ZX_nuevoTrabajo=function(){
  if(!puedeCrearTrabajo()){alert("No tienes permiso para crear trabajos."); return}
  formulario({});
};

window.ZX_editarTrabajo=async function(id){
  if(!puedeEditarTrabajo()){alert("No tienes permiso para editar trabajos."); return}
...
window.ZX_borrarTrabajo=async function(id){
  if(!puedeArchivarTrabajo() && !puedeBorrarTrabajo()){alert("No tienes permiso para gestionar trabajos."); return}
  ...
  document.body.insertAdjacentHTML("beforeend",`
    <div id="zx_modal_trabajo" class="zx_modal_fondo">
      <div class="zx_modal_caja">
        ...
        <div class="zx_text"><b>${limpiar(trabajo.titulo || "Trabajo")}</b><br>Cliente: ${limpiar(trabajo.cliente || "-")}</div>
        ${puedeArchivarTrabajo() ? (trabajo.archivado ? `<button class="zx_btn_big zx_verde" id="tr_restaurar">Restaurar trabajo</button>` : `<button class="zx_btn_big zx_naranja" id="tr_archivar">Archivar trabajo</button>`) : ""}
  ...
  if(archivar) archivar.onclick=async function(){
    const ok=await pedirPinAdmin();
    if(!ok) return;
    const r=await sb().from("trabajos").update({archivado:true}).eq("id",id);
    if(r.error){alert("Error archivando trabajo: "+r.error.message); return}
    await sb().from("agenda_eventos").delete().eq("origen","trabajos").eq("origen_id",String(id));
    await registrarAuditoriaTrabajo(trabajo,"ARCHIVAR_TRABAJO",{estado:trabajo.estado || "",prioridad:trabajo.prioridad || ""});
    cerrarModalTrabajo();
    ZX_trabajos();
  };

  const restaurar=document.getElementById("tr_restaurar");
  if(restaurar) restaurar.onclick=async function(){
    const ok=await pedirPinAdmin();
    if(!ok) return;
    const r=await sb().from("trabajos").update({archivado:false}).eq("id",id);
    if(r.error){alert("Error restaurando trabajo: "+r.error.message); return}
    const plan=await cargarPlanificacion(id);
    await sincronizarAgenda(id,{...trabajo,archivado:false},plan);
    await registrarAuditoriaTrabajo(trabajo,"RESTAURAR_TRABAJO",{estado:trabajo.estado || "",prioridad:trabajo.prioridad || ""});
    cerrarModalTrabajo();
    ZX_trabajos();
  };
  ...
  if(borrar) borrar.onclick=async function(){
    ...
    await registrarAuditoriaTrabajo(trabajo,"BORRAR_TRABAJO",{estado:trabajo.estado || "",prioridad:trabajo.prioridad || "",cliente:trabajo.cliente || ""});
    await sb().from("agenda_eventos").delete().eq("origen","trabajos").eq("origen_id",String(id));
    ...
    if(r.error){alert("Error borrando trabajo: "+r.error.message); return}
    cerrarModalTrabajo();
    ZX_trabajos();
  };
};
...
window.ZX_cambiarEstadoTrabajo=async function(id){
  if(!puedeCambiarEstadoTrabajo()){alert("No tienes permiso para cambiar estados."); return}
  const r0=await sb().from("trabajos").select("*").eq("id",id).maybeSingle();
  if(r0.error || !r0.data){alert("Trabajo no encontrado."); return}
  ...
  const r=await sb().from("trabajos").update({estado:nuevo}).eq("id",id);
  ...
  await registrarAuditoriaTrabajo(r0.data,"CAMBIAR_ESTADO",{anterior:actual,nuevo:nuevo});
  ZX_trabajos();
};
  ...
  return `
    <div class="zx_card">
      <div class="zx_tr_head">
        <div><h2>Trabajos</h2><div class="zx_text">Activos: <b>${activos}</b> · Pendientes: <b>${pendientes}</b> · En curso: <b>${curso}</b><br>Terminados: <b>${terminados}</b> · Urgentes: <b>${urgentes}</b> · Archivados: <b>${archivados}</b></div></div>
...
function textoListaMateriales(trabajo,materiales){
  const lineas=["LISTA DE MATERIALES","","Trabajo: "+String(trabajo.titulo || "-"),"Cliente: "+String(trabajo.cliente || "-"),"Dirección: "+String(direccionTrabajo(trabajo) || "-"),"","MATERIALES:"];
  if(!materiales.length) lineas.push("- Sin materiales.");
  else materiales.forEach((m,i)=>lineas.push((i+1)+". "+String(m.material || "Material")+" ("+[m.cantidad || "0",m.unidad || ""].filter(Boolean).join(" ")+")"+(m.notas ? " · "+m.notas : "")));
  return lineas.join("\n");
}

function htmlListaMateriales(trabajo,materiales){
  return `<!DOCTYPE html><html lang="es"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Lista de materiales</title><style>body{font-family:Arial,sans-serif;margin:24px;color:#0f172a}h1{font-size:26px;margin:0 0 8px}h2{font-size:18px;margin:22px 0 10px}.meta{font-size:14px;line-height:1.45;color:#334155;margin-bottom:16px}.item{border:1px solid #d1d5db;border-radius:12px;padding:12px;margin:8px 0;display:grid;grid-template-columns:28px 1fr;gap:8px}.box{width:20px;height:20px;border:2px solid #0f172a;border-radius:4px;margin-top:1px}b{font-size:16px}span{display:block;color:#334155;font-size:14px;margin-top:3px}small{display:block;color:#64748b;font-size:13px;margin-top:4px}@media print{body{margin:12mm}.item{break-inside:avoid}}</style></head><body><h1>Lista de materiales</h1><div class="meta"><b>Trabajo:</b> ${limpiar(trabajo.titulo || "-")}<br><b>Cliente:</b> ${limpiar(trabajo.cliente || "-")}<br><b>Dirección:</b> ${limpiar(direccionTrabajo(trabajo) || "-")}</div><h2>Materiales</h2>${materiales.length ? materiales.map(m=>`<div class="item"><div class="box"></div><div><b>${limpiar(m.material || "Material")}</b><span>${limpiar(m.cantidad || "0")} ${limpiar(m.unidad || "")}</span>${m.notas ? `<small>${limpiar(m.notas)}</small>` : ""}</div></div>`).join("") : `<div>Sin materiales.</div>`}<script>window.onload=function(){setTimeout(function(){window.print()},250)};<\/script></body></html>`;
}
...
window.ZX_tr_share_materiales=async function(trabajoId){
  ...
  if(navigator.share){
    try{await navigator.share({title:"Lista de materiales - "+String(datos.trabajo.titulo || "Trabajo"),text:texto}); return}catch(e){}
  }
  try{await navigator.clipboard.writeText(texto); alert("Lista copiada al portapapeles.")}
  catch(e){location.href="mailto:?subject="+encodeURIComponent("Lista de materiales - "+String(datos.trabajo.titulo || "Trabajo"))+"&body="+encodeURIComponent(texto)}
};
...
window.ZX_trabajos=async function(){
  ...
  const tarjetas=[];
  for(const t of datos) tarjetas.push(await renderTrabajo(t));
  ...
  document.querySelectorAll("[data-tr-filtro]").forEach(btn=>{btn.onclick=function(){ZX_TR_FILTRO=btn.dataset.trFiltro || "activos"; ZX_trabajos()}});
  ...
  if(buscador){
    buscador.oninput=function(){
      ...
      ZX_TR_TIMER=setTimeout(function(){ZX_trabajos()},250);
    };
  }
  ...
  document.querySelectorAll("[data-mat-preparado]").forEach(chk=>{chk.onchange=function(){cambiarPreparadoMaterial(chk.dataset.matPreparado,chk.checked)}});
...
(function estilosTrabajos(){
  if(document.getElementById("zx_trabajos_v3115")) return;
  const s=document.createElement("style");
  s.id="zx_trabajos_v3115";
  s.innerHTML=`
    ...
    .zx_tr_grid2,.zx_plan_form_grid{display:grid;grid-template-columns:1fr 1fr;gap:10px}.zx_plan_form_grid{grid-template-columns:1.2fr .9fr .9fr}
    .zx_tr_plan_form,.zx_tr_panel{background:#f8fafc;border:1px solid #d1d5db;border-radius:20px;padding:14px;margin-top:14px}
    .zx_plan_details{margin-top:10px}.zx_plan_details summary{font-weight:900;color:#2563eb;cursor:pointer}.zx_plan_buttons{display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-top:10px}
    .zx_plan_compact{display:grid;gap:8px}.zx_plan_row{display:grid;grid-template-columns:1fr 1fr 1.2fr;gap:8px;align-items:center;background:white;border:1px solid #e5e7eb;border-radius:14px;padding:10px;font-weight:850;color:#0f172a}.zx_plan_fecha{color:#334155}.zx_plan_hora{color:#2563eb}.zx_plan_operario{color:#0f172a}.zx_plan_notas{grid-column:1/-1;color:#64748b;font-size:14px;font-weight:750}
    .zx_tr_file_item,.zx_tr_mat_item,.zx_tr_hist_item{background:white;border:1px solid #e5e7eb;border-radius:16px;padding:12px;margin-top:10px}.zx_tr_file_item{display:grid;grid-template-columns:1fr auto;gap:10px;align-items:center}.zx_tr_mat_item span{color:#64748b;font-weight:800}
    .zx_tr_card{border-width:3px}.zx_tr_card_urgente{border-color:#dc2626;box-shadow:0 8px 28px rgba(220,38,38,.18)}.zx_tr_card_alta{border-color:#ea580c;box-shadow:0 8px 28px rgba(234,88,12,.14)}.zx_tr_card_bloqueado{border-color:#991b1b;box-shadow:0 8px 28px rgba(153,27,27,.22)}.zx_tr_card_terminado{border-color:#16a34a}.zx_tr_card_archivado{border-color:#64748b;opacity:.82}
    .zx_tr_badges{display:flex;flex-wrap:wrap;gap:8px;margin:8px 0 12px}.zx_badge{display:inline-flex;align-items:center;justify-content:center;border-radius:999px;padding:8px 12px;color:white;font-size:14px;font-weight:900;text-transform:uppercase;letter-spacing:.3px}.zx_estado_pendiente{background:#ea580c!important;color:white!important}.zx_estado_curso{background:#2563eb!important;color:white!important}.zx_estado_terminado{background:#16a34a!important;color:white!important}.zx_estado_bloqueado{background:#991b1b!important;color:white!important}.zx_estado_neutro{background:#64748b!important;color:white!important}.zx_prio_baja{background:#64748b}.zx_prio_media{background:#2563eb}.zx_prio_alta{background:#ea580c}.zx_prio_urgente{background:#dc2626}.zx_archivado_badge{background:#334155}
    .zx_user_card{background:white;border:1px solid #d1d5db;border-radius:24px;padding:22px;margin:18px 0;box-shadow:0 8px 24px rgba(0,0,0,.04)}.zx_user_name{font-size:30px;font-weight:900;color:#0f172a;margin-bottom:10px}.zx_user_data{color:#334155;font-size:18px;line-height:1.55;font-weight:700}.zx_user_actions{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px;margin-top:14px}.zx_action_btn{border:0;border-radius:16px;background:#e5e7eb;padding:14px;font-size:17px;font-weight:900;color:#111827}.zx_blue{background:#2563eb!important;color:white!important}.zx_red{background:#dc2626!important;color:white!important}.zx_verde{background:#16a34a!important;color:white!important}.zx_label{display:block;margin:12px 0 6px;color:#334155;font-size:15px;font-weight:900}.zx_form_subtitle{margin:22px 0 8px;color:#0f172a;font-size:24px;font-weight:900}
    .zx_obra_card{background:white;border:3px solid #d1d5db;border-radius:24px;padding:16px;margin:16px 0;box-shadow:0 8px 24px rgba(0,0,0,.05)}.zx_obra_hora{color:#2563eb;font-size:18px;font-weight:900;margin-bottom:4px}.zx_obra_titulo{color:#0f172a;font-size:26px;font-weight:950;line-height:1.12}.zx_obra_cliente{color:#64748b;font-size:16px;font-weight:900;margin-top:4px}.zx_obra_big_btn{border:0;border-radius:18px;background:#2563eb;color:white;padding:17px 12px;font-size:17px;font-weight:950;text-align:center}.zx_obra_section{background:#f8fafc;border:1px solid #d1d5db;border-radius:18px;padding:12px;margin-top:12px}.zx_obra_section summary{cursor:pointer;color:#0f172a;font-size:18px;font-weight:950}.zx_obra_file_btn{width:100%;border:1px solid #e5e7eb;background:white;border-radius:16px;padding:13px;margin-top:8px;text-align:left;color:#0f172a}.zx_obra_file_btn b{display:block;font-size:15px;font-weight:950}.zx_obra_file_btn span{display:block;color:#64748b;font-size:13px;font-weight:850;margin-top:3px}
    .zx_obra_compacta{padding:14px}.zx_obra_top_compacta{display:grid;grid-template-columns:1fr auto;gap:10px;align-items:start;margin-bottom:10px}.zx_obra_estado_material{border-radius:18px;padding:10px 12px;text-align:center;min-width:78px;border:1px solid #e5e7eb}.zx_obra_estado_material b{display:block;font-size:24px;font-weight:950;color:#0f172a;line-height:1}.zx_obra_estado_material span{display:block;font-size:11px;font-weight:900;color:#64748b;margin-top:4px;text-transform:uppercase}.zx_mat_estado_ok{background:#ecfdf5!important;border-color:#86efac!important}.zx_mat_estado_ok b{color:#16a34a!important}.zx_mat_estado_pendiente{background:#fff7ed!important;border-color:#fed7aa!important}.zx_mat_estado_pendiente b{color:#ea580c!important}.zx_mat_estado_vacio{background:#f8fafc!important;border-color:#e5e7eb!important}
    .zx_obra_material_preparacion{border:1px solid #e5e7eb;border-radius:20px;padding:12px;margin:10px 0 12px}.zx_mat_prepa_head{display:grid;grid-template-columns:1fr auto;gap:10px;align-items:center;margin-bottom:10px}.zx_mat_prepa_head b{display:block;color:#0f172a;font-size:20px;font-weight:950;line-height:1.15}.zx_mat_prepa_head span{display:block;color:#64748b;font-size:13px;font-weight:900;margin-top:3px}.zx_mat_prepa_lista{display:grid;gap:7px;margin-bottom:10px;max-height:220px;overflow:auto;padding-right:2px}.zx_mat_prepa_item{display:grid;grid-template-columns:auto 1fr;gap:10px;align-items:start;background:white;border:1px solid #e5e7eb;border-radius:14px;padding:10px}.zx_mat_prepa_item input{width:23px!important;height:23px!important;margin-top:2px!important}.zx_mat_prepa_item b{display:block;color:#0f172a;font-size:15px;font-weight:950}.zx_mat_prepa_item em{display:block;color:#2563eb;font-style:normal;font-size:13px;font-weight:900;margin-top:2px}.zx_mat_prepa_item small{display:block;color:#64748b;font-size:12px;font-weight:800;margin-top:4px}.zx_mat_prepa_item_ok{opacity:.66}.zx_mat_prepa_item_ok b{text-decoration:line-through}.zx_obra_acciones_rapidas{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:9px;margin:10px 0}.zx_obra_was{background:#16a34a!important}.zx_obra_datos_compactos{display:grid;grid-template-columns:1fr 1fr;gap:8px}.zx_obra_datos_compactos div{background:white;border:1px solid #e5e7eb;border-radius:14px;padding:10px}.zx_obra_datos_compactos b{display:block;color:#64748b;font-size:12px;font-weight:900;text-transform:uppercase}.zx_obra_datos_compactos span{display:block;color:#0f172a;font-size:14px;font-weight:850;margin-top:4px;line-height:1.25}
    .zx_admin_materiales_lista{display:grid;gap:8px}.zx_admin_mat_check{display:grid;grid-template-columns:auto 1fr;gap:10px;align-items:start}.zx_admin_mat_check input{width:24px!important;height:24px!important;margin-top:2px!important}.zx_admin_mat_check b{display:block;color:#0f172a;font-size:16px;font-weight:950}.zx_admin_mat_check em{display:block;color:#2563eb;font-style:normal;font-size:14px;font-weight:900;margin-top:2px}.zx_admin_mat_check small{display:block;color:#64748b;font-size:12px;font-weight:800;margin-top:3px}.zx_tr_mat_preparado{background:#ecfdf5!important;border-color:#86efac!important}.zx_tr_mat_preparado b{text-decoration:line-through;color:#166534!important}
    .zx_obra_material_actions{display:grid;grid-template-columns:1fr 1fr;gap:8px;margin:10px 0}.zx_obra_material_tabs{display:grid;grid-template-columns:1fr 1fr;gap:8px;margin:8px 0 16px}.zx_obra_material_tabs button{border:0;border-radius:16px;background:#e5e7eb;color:#0f172a;padding:14px 10px;font-size:15px;font-weight:950}.zx_obra_material_tabs .zx_mat_tab_on{background:#2563eb!important;color:white!important}.zx_obra_material_modal_head{background:#f8fafc;border:1px solid #e5e7eb;border-radius:16px;padding:12px;margin:10px 0 14px}.zx_obra_material_modal_head b{display:block;color:#0f172a;font-size:17px;font-weight:950}.zx_obra_material_modal_head span{display:block;color:#64748b;font-size:14px;font-weight:850;margin-top:3px}.zx_mat_lista_compacta{display:grid;gap:8px;margin:10px 0 12px}.zx_mat_linea_compacta{display:grid;grid-template-columns:auto 1fr;gap:10px;align-items:start;background:white;border:1px solid #e5e7eb;border-radius:16px;padding:10px;cursor:pointer}.zx_mat_linea_compacta input{width:24px!important;height:24px!important;margin-top:2px!important}.zx_mat_linea_texto{border:0;background:transparent;text-align:left;padding:0;width:100%;display:block}.zx_mat_linea_texto b{display:block;color:#0f172a;font-size:15px;font-weight:950}.zx_mat_linea_texto em{display:block;color:#2563eb;font-size:13px;font-style:normal;font-weight:900;margin-top:2px}.zx_mat_linea_texto small{display:block;color:#64748b;font-size:12px;font-weight:800;margin-top:4px}.zx_mat_linea_ok{background:#f8fafc}.zx_mat_linea_ok b{text-decoration:line-through;color:#64748b}.zx_mat_linea_selected{border-color:#2563eb!important;box-shadow:0 0 0 3px rgba(37,99,235,.16);background:#eff6ff!important}.zx_mat_acciones_seleccion{position:sticky;bottom:0;z-index:10;background:white;border:2px solid #bfdbfe;border-radius:18px;padding:12px;margin:12px 0 6px;box-shadow:0 -8px 24px rgba(15,23,42,.12)}.zx_mat_acciones_seleccion b{display:block;color:#0f172a;font-size:15px;font-weight:950}.zx_mat_acciones_seleccion span{display:block;color:#64748b;font-size:12px;font-weight:850;margin-top:3px}.zx_mat_acciones_botones{display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-top:10px}.zx_mat_acciones_ocultas{display:none!important}
    @media(max-width:430px){.zx_tr_head,.zx_tr_grid2,.zx_plan_form_grid,.zx_user_actions,.zx_tr_file_item,.zx_plan_row{grid-template-columns:1fr;display:grid}.zx_user_card{padding:16px;border-radius:20px}.zx_user_name{font-size:24px}.zx_user_data{font-size:16px}}
  `;
