...
function fechaCorta(f){const d=new Date(f);if(isNaN(d))return"-";return String(d.getDate()).padStart(2,"0")+"/"+String(d.getMonth()+1).padStart(2,"0")+"/"+d.getFullYear()+" "+String(d.getHours()).padStart(2,"0")+":"+String(d.getMinutes()).padStart(2,"0")}
function segundosEntre(a,b){const da=new Date(a),db=new Date(b);if(isNaN(da)||isNaN(db))return 0;return Math.max(0,Math.floor((db-da)/1000))}
...
function estadoDesdeTipo(tipo){
  if(tipo==="entrada"||tipo==="fin_descanso"||tipo==="fin_comida")return"dentro";
...
async function abrirMenu(estado){
  ...
  if(sel&&kmEntrada){
    sel.onchange=function(){
      ...
      kmEntrada.value=opt?String(opt.dataset.km||""):"";
    };
  }
...
window.ZX_fichaje_real=async function(){
  ...
  if(est.jornada){
    ...
    objetivoSeg=Number(est.jornada.segundos_objetivo||est.jornada.minutos_objetivo*60||480*60);
  }
  ...
  app().innerHTML=`
    ...
    <div class="zx_card">
      <h2>Mis jornadas</h2>
      ${jornadas.length?jornadas.map(renderJornadaMini).join(""):`<div class="zx_text">Sin jornadas.</div>`}
    </div>
  ...
  if(est.jornada){
    ZX_TIMER=setInterval(()=>actualizarVivo(est.jornada.id,objetivoSeg),1000);
  }
};
