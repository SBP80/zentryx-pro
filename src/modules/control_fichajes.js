...
// V1002 - PANEL RESUMIDO / ESTADO EN VIVO / INCIDENCIAS
// ===============================
(function(){
"use strict";
...
function colorEstado(e){if(e==="abierta")return"#dc2626";if(e==="revisada")return"#f59e0b";if(e==="cerrada")return"#16a34a";return"#64748b"}

async function insertarAviso(usuarioId,titulo,mensaje){
  try{await sb().from("notificaciones").insert([{id:uuidSeguro(),usuario_id:String(usuarioId),titulo,mensaje,tipo:"control_fichajes",leida:false,created_at:ahora()}])}
  catch(e){try{await sb().from("avisos").insert([{id:uuidSeguro(),usuario_id:String(usuarioId),titulo,mensaje,tipo:"control_fichajes",leida:false,created_at:ahora()}])}catch(e2){}}
}
...
async function usuariosActivos(){
  try{
    ...
    return (r.data||[]).filter(u=>u.activo!==false && !["baja","inactivo"].includes(String(u.estado||"").toLowerCase()));
  }catch(e){return []}
}
async function usuarioActualLista(){
  const s=sesion(); return [{id:String(s.id||""),usuario:s.usuario||"",nombre:s.nombre||s.usuario||"",rol:s.rol||"",estado:"activo",activo:true}]
}
...
async function jornadasUsuarioFecha(usuarioId,fecha){
  try{const r=await sb().from("jornadas").select("*").eq("usuario_id",String(usuarioId)).eq("fecha",String(fecha).slice(0,10)).order("created_at",{ascending:false});return r.error?[]:(r.data||[])}catch(e){return []}
}
async function jornadaAbiertaUsuario(usuarioId){
  try{const r=await sb().from("jornadas").select("*").eq("usuario_id",String(usuarioId)).eq("estado","abierta").order("created_at",{ascending:false}).limit(1);return (!r.error&&r.data&&r.data.length)?r.data[0]:null}catch(e){return null}
}
async function fichajesDeJornada(jid){
  try{const r=await sb().from("fichajes").select("*").eq("jornada_id",String(jid)).order("created_at",{ascending:true});return r.error?[]:(r.data||[])}catch(e){return []}
}
async function incidenciasControl(usuarioId=null){
  try{let q=sb().from("control_fichajes_incidencias").select("*").order("created_at",{ascending:false}).limit(80); if(usuarioId)q=q.eq("usuario_id",String(usuarioId)); const r=await q; return r.error?[]:(r.data||[])}catch(e){return []}
}
async function incidenciaExistente(usuarioId,fecha,tipo,jornadaId){
  try{
    ...
    const r=await q; return (!r.error&&r.data&&r.data.length)?r.data[0]:null;
  }catch(e){return null}
}
...
async function evaluarUsuario(u){
  const fecha=fechaHoyISO(), cfg=await configUsuario(u.id), out={usuario:u,cfg,estado:"correcto",incidencias:[]};
  ...
  const actual=minutosAhora(), entrada=minutosDesdeHora(cfg.hora_entrada), salida=minutosDesdeHora(cfg.hora_salida);
  ...
  if(abierta){
    const ev=await fichajesDeJornada(abierta.id), estado=estadoDesdeEventos(ev);
    ...
    if(abierta.entrada){
      const minAbierta=minutosDesdeISO(abierta.entrada), max=Number(cfg.max_jornada_horas||12)*60;
      if(minAbierta>max){
    ...
    if(estado==="descanso"){
      const ev0=ultimoEvento(ev,"inicio_descanso"), min=minutosDesdeISO(ev0?ev0.created_at:null), max=Number(cfg.max_descanso_min||30);
      if(min>max){
    ...
    if(estado==="comida"){
      const ev0=ultimoEvento(ev,"inicio_comida"), min=minutosDesdeISO(ev0?ev0.created_at:null), max=Number(cfg.max_comida_min||60);
      if(min>max){
...
async function evaluarTodos(){
  ...
  const out=[]; for(const u of lista){if(u.id)out.push(await evaluarUsuario(u))}
  return out;
}

function renderGeneral(res){
  const inc=res.filter(r=>r.estado==="incidencia").length, ok=res.filter(r=>r.estado==="correcto").length, off=res.filter(r=>r.estado==="sin_control").length;
  const color=inc?"#dc2626":"#16a34a", texto=inc?(inc+" incidencia(s) activa(s)"):"Todo correcto";
  return `<div class="zx_card"><h2>Control de fichajes</h2><div style="font-size:30px;font-weight:900;color:${color};margin-top:8px;">${texto}</div><div class="zx_text" style="margin-top:10px;">Fecha: <b>${formatoFechaES(fechaHoyISO())}</b><br>Usuarios revisados: <b>${res.length}</b><br>Correctos: <b>${ok}</b><br>Sin control: <b>${off}</b></div><button class="zx_btn_big zx_azul" id="zx_cf_revisar">Revisar ahora</button></div>`;
}
function renderUsuario(r){
  let c="#16a34a", e="Correcto"; if(r.estado==="incidencia"){c="#dc2626";e="Incidencia"} if(r.estado==="sin_control"){c="#64748b";e="Sin control"}
  return `<div class="zx_cf_usuario"><div class="zx_cf_usuario_top"><b>${limpiar(r.usuario.nombre||r.usuario.usuario||"-")}</b><span style="background:${c};">${e}</span></div><div class="zx_admin_data">Entrada: ${limpiar(r.cfg.hora_entrada)} + ${limpiar(r.cfg.margen_entrada_min)} min · Salida: ${limpiar(r.cfg.hora_salida)} + ${limpiar(r.cfg.margen_salida_min)} min<br>Descanso máx.: ${limpiar(r.cfg.max_descanso_min)} min · Comida máx.: ${limpiar(r.cfg.max_comida_min)} min · Jornada máx.: ${limpiar(r.cfg.max_jornada_horas)} h</div>${r.incidencias.length?r.incidencias.map(i=>`<div class="zx_cf_inc_line"><b>${limpiar(textoInc(i.tipo))}</b><br>${limpiar(i.detalle||"")}</div>`).join(""):`<div class="zx_admin_data">Sin incidencias nuevas.</div>`}</div>`;
}
function renderIncidencia(i){
  return `<div class="zx_admin_row"><div class="zx_admin_row_top"><b>${limpiar(i.nombre||i.usuario||"-")}</b><span>${limpiar(formatoFechaES(i.fecha))}</span></div><div class="zx_cf_badge" style="background:${colorEstado(i.estado)};">${limpiar(i.estado||"")}</div><div class="zx_admin_data"><b>${limpiar(textoInc(i.tipo))}</b><br>${limpiar(i.detalle||"")}<br>Creada: ${limpiar(fechaHoraES(i.created_at))}${i.minutos_exceso!=null?`<br>Exceso: ${limpiar(i.minutos_exceso)} min`:""}${i.comentario_admin?`<br>Comentario: ${limpiar(i.comentario_admin)}`:""}</div>${esAdmin()?`<div class="zx_edit_grid"><button class="zx_admin_btn zx_admin_editar" data-cf-revisar="${i.id}">Revisada</button><button class="zx_admin_btn zx_admin_borrar" data-cf-cerrar="${i.id}">Cerrar</button></div>`:""}</div>`;
}
function renderConfig(u,c){
  return `<div class="zx_admin_row"><div class="zx_admin_row_top"><b>${limpiar(u.nombre||u.usuario||"-")}</b><span>${limpiar(u.rol||"")}</span></div><label class="zx_label">Activo</label><select data-cfg-activo="${u.id}"><option value="true" ${c.activo?"selected":""}>Sí</option><option value="false" ${!c.activo?"selected":""}>No</option></select><div class="zx_cf_grid2"><div><label class="zx_label">Hora entrada</label><input type="time" data-cfg-entrada="${u.id}" value="${limpiar(c.hora_entrada)}"></div><div><label class="zx_label">Hora salida</label><input type="time" data-cfg-salida="${u.id}" value="${limpiar(c.hora_salida)}"></div></div><div class="zx_cf_grid2"><div><label class="zx_label">Margen entrada min</label><input type="number" data-cfg-margen-entrada="${u.id}" value="${limpiar(c.margen_entrada_min)}"></div><div><label class="zx_label">Margen salida min</label><input type="number" data-cfg-margen-salida="${u.id}" value="${limpiar(c.margen_salida_min)}"></div></div><div class="zx_cf_grid2"><div><label class="zx_label">Descanso máx. min</label><input type="number" data-cfg-descanso="${u.id}" value="${limpiar(c.max_descanso_min)}"></div><div><label class="zx_label">Comida máx. min</label><input type="number" data-cfg-comida="${u.id}" value="${limpiar(c.max_comida_min)}"></div></div><label class="zx_label">Jornada máxima horas</label><input type="number" data-cfg-jornada="${u.id}" value="${limpiar(c.max_jornada_horas)}"><button class="zx_admin_btn zx_admin_editar" data-cfg-guardar="${u.id}">Guardar configuración</button></div>`;
}

function estilos(){
  if(document.getElementById("zx_control_fichajes_css"))return;
  const s=document.createElement("style"); s.id="zx_control_fichajes_css";
  s.innerHTML=`.zx_admin_row,.zx_cf_usuario{background:#f8fafc;border:1px solid #d1d5db;border-radius:18px;padding:14px;margin-top:10px}.zx_admin_row_top,.zx_cf_usuario_top{display:flex;justify-content:space-between;gap:8px;font-size:16px;color:#0f172a;font-weight:900}.zx_admin_row_top span{color:#64748b;font-size:14px;white-space:nowrap}.zx_admin_data{color:#64748b;font-size:15px;line-height:1.45;font-weight:800;word-break:break-word;margin-top:8px}.zx_admin_btn{width:100%;border:0;border-radius:14px;margin-top:10px;padding:12px;color:white;font-size:16px;font-weight:900}.zx_admin_editar{background:#2563eb}.zx_admin_borrar{background:#dc2626}.zx_edit_grid,.zx_cf_grid2{display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-top:10px}.zx_label{display:block;margin-top:14px;margin-bottom:6px;color:#64748b;font-weight:900;font-size:15px}.zx_admin_row input,.zx_admin_row select{width:100%;border:1px solid #cbd5e1;border-radius:14px;padding:12px;font-size:16px;font-weight:800;color:#0f172a;background:#f8fafc}.zx_cf_usuario_top span,.zx_cf_badge{display:inline-block;color:white;border-radius:999px;padding:5px 10px;font-size:13px;font-weight:900}.zx_cf_inc_line{background:#fee2e2;color:#7f1d1d;border-radius:14px;padding:10px;margin-top:10px;font-weight:800;line-height:1.35}`;
  document.head.appendChild(s);
}
async function guardarConfigPantalla(id){
  const d={activo:document.querySelector(`[data-cfg-activo="${id}"]`)?.value==="true",hora_entrada:document.querySelector(`[data-cfg-entrada="${id}"]`)?.value||"08:00",hora_salida:document.querySelector(`[data-cfg-salida="${id}"]`)?.value||"17:00",margen_entrada_min:document.querySelector(`[data-cfg-margen-entrada="${id}"]`)?.value||15,margen_salida_min:document.querySelector(`[data-cfg-margen-salida="${id}"]`)?.value||15,max_descanso_min:document.querySelector(`[data-cfg-descanso="${id}"]`)?.value||30,max_comida_min:document.querySelector(`[data-cfg-comida="${id}"]`)?.value||60,max_jornada_horas:document.querySelector(`[data-cfg-jornada="${id}"]`)?.value||12};
  if(await guardarConfigUsuario(id,d)){alert("Configuración guardada.");ZX_control_fichajes()}
}
function enlazar(){
  const b=document.getElementById("zx_cf_revisar"); if(b)b.onclick=()=>ZX_control_fichajes();
...
window.ZX_control_fichajes=async function(){
  estilos(); if(ZX_CF_TIMER){clearInterval(ZX_CF_TIMER);ZX_CF_TIMER=null}
  document.querySelectorAll(".zx_nav_btn").forEach(b=>{b.classList.remove("zx_activo");if(b.dataset.modulo==="control_fichajes")b.classList.add("zx_activo")});
  const res=await evaluarTodos(), s=sesion(), incs=await incidenciasControl(esAdmin()?null:s.id);
  let configs=[]; if(esAdmin()){const us=await usuariosActivos(); for(const u of us)configs.push({u,c:await configUsuario(u.id)})}
  app().innerHTML=`${renderGeneral(res)}<div class="zx_card"><h2>Estado de hoy</h2>${res.length?res.map(renderUsuario).join(""):`<div class="zx_text">Sin usuarios para revisar.</div>`}</div><div class="zx_card"><button class="zx_btn_big zx_gris" onclick="ZX_CF_toggleIncidencias()">${ZX_CF_VER_INCIDENCIAS?"Ocultar incidencias":"Ver incidencias"}</button>${ZX_CF_VER_INCIDENCIAS?(incs.length?incs.map(renderIncidencia).join(""):`<div class="zx_text">Sin incidencias.</div>`):""}</div>${esAdmin()?`<div class="zx_card"><button class="zx_btn_big zx_gris" onclick="ZX_CF_toggleConfig()">${ZX_CF_VER_CONFIG?"Ocultar configuración":"Ver configuración"}</button>${ZX_CF_VER_CONFIG?(configs.length?configs.map(x=>renderConfig(x.u,x.c)).join(""):`<div class="zx_text">Sin usuarios.</div>`):""}</div>`:""}`;
  enlazar();
  ZX_CF_TIMER=setInterval(async()=>{const mod=document.querySelector(".zx_nav_btn.zx_activo")?.dataset?.modulo;if(mod==="control_fichajes"){const r=await evaluarTodos();if(r.some(x=>x.estado==="incidencia"))ZX_control_fichajes()}},60000);
};
