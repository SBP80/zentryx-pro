...
// V1006 - VISUAL COMPACTO / HORAS SIN SEGUNDOS
// ===============================
(function(){
"use strict";
...
let ZX_CF_DET_USUARIO={};
let ZX_CF_DET_CONFIG={};
let ZX_CF_TIMER=null;
let ZX_CF_SCROLL=null;
...
function guardarScroll(){
  ZX_CF_SCROLL=window.scrollY||document.documentElement.scrollTop||0;
}

function restaurarScroll(){
  if(ZX_CF_SCROLL===null)return;
  const y=ZX_CF_SCROLL;
  ZX_CF_SCROLL=null;
...
async function usuariosActivos(){
  try{
    ...
    return (r.data||[]).filter(u=>{
      ...
      return u.activo!==false && e!=="baja" && e!=="inactivo";
    });
  }catch(e){return[]}
}
...
async function configsTodos(){
  try{
    const r=await sb().from("control_fichajes_config").select("*");
...
async function guardarConfigUsuario(usuarioId,d){
  ...
  const reg={
    ...
    usuario_id:String(usuarioId),
    activo:!!d.activo,
    hora_entrada:horaCorta(d.hora_entrada||"08:00"),
    hora_salida:horaCorta(d.hora_salida||"17:00"),
...
async function fichajesHoyTodas(fecha){
  try{
    const r=await sb()
      .from("fichajes")
      .select("*")
      .gte("created_at",String(fecha).slice(0,10)+"T00:00:00")
...
async function evaluarUsuarioConCache(u,cfg,jornadasHoy,abiertas,fichajesHoy){
  ...
  if(!tieneEntrada && entrada!==null && actual>=entrada+Number(cfg.margen_entrada_min||0)){
    const inc=await crearIncidencia({
      ...
      detalle:(u.nombre||u.usuario||"Usuario")+" no ha fichado entrada. Prevista: "+horaCorta(cfg.hora_entrada)+" + "+cfg.margen_entrada_min+" min."
    },cfg);

    if(inc){
      out.incidencias.push(inc);
  ...
  if(abierta){
    ...
    const ev=(fichajesHoy||[])
      .filter(f=>String(f.jornada_id)===String(abierta.id))
    ...
    if(salida!==null && actual>=salida+Number(cfg.margen_salida_min||0)){
      const inc=await crearIncidencia({
        ...
        detalle:(u.nombre||u.usuario||"Usuario")+" tiene jornada abierta pasada la salida prevista: "+horaCorta(cfg.hora_salida)+".",
...
async function evaluarTodos(){
  ...
  const configs=datos[0];
  const jornadasHoy=datos[1];
  const abiertas=datos[2];
  const fichajesHoy=datos[3];
...
function renderUsuario(r){
  ...
  const abierto=!!ZX_CF_DET_USUARIO[uid];
  ...
  return `
    <div class="zx_cf_usuario">
      ...
      ${abierto?`
        <div class="zx_admin_data zx_cf_detalle_limpio">
          <div><b>Entrada</b><span>${limpiar(horaCorta(r.cfg.hora_entrada))} + ${limpiar(r.cfg.margen_entrada_min)} min</span></div>
          <div><b>Salida</b><span>${limpiar(horaCorta(r.cfg.hora_salida))} + ${limpiar(r.cfg.margen_salida_min)} min</span></div>
          <div><b>Descanso</b><span>${limpiar(r.cfg.max_descanso_min)} min</span></div>
          <div><b>Comida</b><span>${limpiar(r.cfg.max_comida_min)} min</span></div>
          <div><b>Jornada máx.</b><span>${limpiar(r.cfg.max_jornada_horas)} h</span></div>
        </div>
...
function renderIncidencia(i){
  return `
    <div class="zx_admin_row">
      ...
      <div class="zx_cf_badge" style="background:${colorIncidencia(i.estado)};">
        ${limpiar(i.estado||"")}
      </div>
      ...
      ${
        ...
        ? `<div class="zx_edit_grid">
            <button class="zx_admin_btn zx_admin_editar" data-cf-revisar="${i.id}">Revisada</button>
            <button class="zx_admin_btn zx_admin_borrar" data-cf-cerrar="${i.id}">Cerrar</button>
          </div>`
        : ""
      }
    </div>
...
function renderConfig(u,c){
  ...
  const abierto=!!ZX_CF_DET_CONFIG[uid];
    ...
    <div class="zx_admin_row">
      ...
      ${abierto?`
        ...
        <div class="zx_cf_grid2">
          <div><label class="zx_label">Entrada</label><input type="time" data-cfg-entrada="${u.id}" value="${limpiar(horaCorta(c.hora_entrada))}"></div>
          <div><label class="zx_label">Salida</label><input type="time" data-cfg-salida="${u.id}" value="${limpiar(horaCorta(c.hora_salida))}"></div>
        </div>
...
function estilos(){
  ...
  s.innerHTML=`
    ...
    .zx_cf_detalle_limpio{display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-top:12px}
    .zx_cf_detalle_limpio div{background:white;border:1px solid #dbe3ee;border-radius:12px;padding:9px}
    .zx_cf_detalle_limpio b{display:block;color:#334155;font-size:13px;font-weight:900}
    .zx_cf_detalle_limpio span{display:block;color:#0f172a;font-size:15px;font-weight:900;margin-top:2px}
    ...
    @media(min-width:700px){
      .zx_cf_resumen_grid{grid-template-columns:repeat(3,1fr)}
    }
  `;
...
async function guardarConfigPantalla(id){
  ...
  if(await guardarConfigUsuario(id,d)){
    ZX_CF_DET_CONFIG[String(id)]=false;
    ZX_control_fichajes();
  }
}
...
function enlazar(){
  ...
  document.querySelectorAll("[data-cf-toggle-usuario]").forEach(b=>{
    b.onclick=function(){
      ...
      ZX_CF_DET_USUARIO[id]=!ZX_CF_DET_USUARIO[id];
      ZX_control_fichajes();
    };
  });
  ...
  document.querySelectorAll("[data-cfg-toggle]").forEach(b=>{
    b.onclick=function(){
      ...
      ZX_CF_DET_CONFIG[id]=!ZX_CF_DET_CONFIG[id];
      ZX_control_fichajes();
    };
  });
...
window.ZX_control_fichajes=async function(){
  ...
  app().innerHTML=`
    <div class="zx_card">
      <h2>Control de fichajes</h2>
      <div class="zx_text">Cargando control...</div>
    </div>
  `;
  ...
  if(esAdmin()){
    ...
    for(const u of us){
      configs.push({
        u,
        c:configUsuarioCache(u.id,confs)
      });
    }
  }
