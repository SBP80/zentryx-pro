...
// V3118 - OBJETIVO FIJADO AL CREAR LA JORNADA
// ===============================
(function(){
"use strict";
...
function objetivoJornadaSeg(j,laboral){
  ...
  if(jornadaTieneSnapshot(j)){
    const baseSeg=Math.max(0,numeroSeguro(j?.config_minutos_dia,0)*60);
    const justificadoSeg=Math.max(0,numeroSeguro(j?.minutos_justificados,0)*60);
    const tipo=String(j?.tipo_ausencia||"");

    if(j?.es_festivo) return 0;
    if(tipo==="vacaciones" || tipo==="baja_medica") return 0;
    if(baseSeg>0 && justificadoSeg>=baseSeg) return 0;

    const guardadoSeg=(j?.segundos_objetivo!==undefined && j?.segundos_objetivo!==null)
      ? Math.max(0,numeroSeguro(j.segundos_objetivo,0))
      : Math.max(0,Math.round(numeroSeguro(j?.minutos_objetivo,0)*60));

    if(guardadoSeg>0) return guardadoSeg;

    // Repara jornadas creadas con snapshot correcto pero objetivo guardado a cero.
    // La reparación usa exclusivamente la copia histórica de esa jornada,
    // nunca la configuración actual del usuario.
    return Math.max(0,baseSeg-justificadoSeg);
  }

  if(j){
    if(j.segundos_objetivo!==undefined && j.segundos_objetivo!==null){
      return Math.max(0,numeroSeguro(j.segundos_objetivo,0));
    }
    if(j.minutos_objetivo!==undefined && j.minutos_objetivo!==null){
      return Math.max(0,Math.round(numeroSeguro(j.minutos_objetivo,0)*60));
    }
  }

  return Math.max(0,numeroSeguro(laboral?.objetivoSeg,0));
}
...
async function crearJornada(motivoAdmin,datosVehiculo=null){
  ...
  const jornadaTemporal={
    ...snapshot,
    es_festivo:!!laboral.festivo,
    tipo_ausencia:laboral.tipoAusencia||null,
    minutos_justificados:Number(laboral.minutosJustificados||0),
    observacion_laboral:laboral.observacion||""
  };

  let objetivoSegInicial=objetivoJornadaSeg(jornadaTemporal,laboral);

  if(cerradas.length){
    objetivoSegInicial=0;
  }

  const objetivoMin=Math.floor(objetivoSegInicial/60);
  ...
  const datos={
    ...
    segundos_objetivo:Math.floor(objetivoSegInicial),
    minutos_extra:0,
    minutos_faltantes:objetivoMin,
    segundos_extra:0,
    segundos_faltantes:Math.floor(objetivoSegInicial),
    horas_extra:0,
