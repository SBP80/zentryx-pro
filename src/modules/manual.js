// V1148 - Saneamiento: "Qué falta para poder dibujarla" se adapta al estado del trazado. Si ya hay puntos o tramos físicos, deja de indicar de forma genérica que el colector no tiene posición, evita repetir la misma tarea y muestra como pendientes el recorrido restante, las entradas/salida sin posición y las cotas que falten.
// V1147 - Saneamiento: la Vista de ejecución muestra sobre la imagen vinculada los puntos y tramos físicos ya confirmados como trazado parcial, sin presentarlos como plano terminado ni prolongar recorridos no registrados.
// V1146 - Saneamiento: el trazado físico sobre una imagen vinculada puede continuar desde el inicio ya guardado de C1 añadiendo puntos sucesivos. Cada punto usa el visor a pantalla completa, conserva como referencia los anteriores, se puede ajustar o quitar el último y no se interpreta por sí solo como salida.
// V1145 - Saneamiento: el posicionamiento sobre plano se abre a pantalla completa; permite zoom con dos dedos, desplazamiento al ampliar, toque o arrastre de una marca precisa y exige ✓ Listo antes de cambiar la posición local. Cancelar conserva la posición anterior y Guardar saneamiento sigue siendo el guardado definitivo.
// V1144 - Saneamiento: Geometría de ejecución permite colocar el primer punto real de un tramo sobre una imagen vinculada; la prueba inicial usa el inicio de C1 y guarda coordenadas porcentuales del propio plano, sin suponer dirección ni recorrido.
// V1143 - Saneamiento: corregida la selección de foto/archivo de la Geometría de ejecución; al elegir un archivo se muestra como seleccionado y sigue pendiente de subida hasta pulsar Guardar, sin alterar posiciones ni recorrido.
// V1142 - Saneamiento: un plano identificado puede vincular una imagen o PDF; el archivo se sube solo al guardar, se reutiliza el bucket existente y la planta física sigue pendiente hasta registrar posiciones reales.
// V1141 - Saneamiento: al elegir un plano o documento como fuente geométrica, Zentryx pide su identificación sin darlo por vinculado ni por suficiente para dibujar; la vista sigue pendiente hasta disponer de posiciones reales.
// V1140 - Saneamiento: Obra y recorrido incorpora Geometría de ejecución para indicar si la planta física se basará en un plano de referencia, referencias medibles o queda marcada como no conocida, para después o no aplicable.
// V1139 - Saneamiento: la lista de elementos distingue En esquema de Sin enlazar; deja de usar En plano para no confundir la topología con una planta física.
// V1138 - Saneamiento: el Asistente de recorrido puede completar una tarjeta existente sin crear otra; conserva lo ya registrado y pregunta solo los datos principales pendientes.
// V1137 - Saneamiento: en la ficha del proyecto, el esquema CTE y cálculo queda plegado; el esquema de conexiones y la vista de ejecución se muestran primero para evitar dos dibujos abiertos con información repetida.
// V1136 - Saneamiento: separa el esquema técnico de conexiones de la vista real de ejecución; no dibuja geometría física sin posiciones o plano de referencia suficiente y muestra qué datos faltan.
// V1135 - Saneamiento: en móvil, la etiqueta del tramo de un colector multientrada queda separada de la última entrada y de la salida para evitar solapes.
// V1134 - Saneamiento: en móvil, varias entradas de un colector se dibujan como ramas independientes hacia una convergencia común para evitar falsas conexiones visuales.
// V1133 - Saneamiento: los colectores admiten varias entradas enlazadas; conserva compatibilidad con el origen único anterior y el plano muestra convergencias sin duplicar la longitud del colector.
// V1132 - Saneamiento: los elementos existentes permiten vincular Desde/Hasta con datos ya registrados; reconoce destinos escritos de forma inequívoca y evita repetir información.
// V1131 - Saneamiento: el plano diferencia elementos ya enlazados de los que todavía no pueden dibujarse; el orden de registro deja de parecer una conexión física.
...
// V1123 - Saneamiento: Ventilación usa vista compacta; solución y recorrido visibles, observaciones/ayuda/comprobación plegadas y grupos técnicos solo cuando corresponden.
...
// V1129 - INTERFAZ MÓVIL V25: ALTURA SOBRE HUECO REALMENTE CONDICIONAL + INDICADORES GLOBALES DE ACORDEÓN MÁS VISIBLES
...
// V1122 - INTERFAZ MÓVIL V18: COMPROBACIÓN TÉCNICA SIN ESTADO REPETIDO Y CON RESÚMENES CORTOS PARA RED, VENTILACIÓN Y PLANO
// V1121 - INTERFAZ MÓVIL V17: COMPROBACIÓN TÉCNICA DE SANEAMIENTO RESUMIDA; DETALLE BAJO DEMANDA
// V1120 - INTERFAZ MÓVIL V16: DIMENSIONADO DE SANEAMIENTO CON DATOS PRINCIPALES VISIBLES Y DATOS ADICIONALES/CRITERIO CTE PLEGADOS
...
// V1115 - INTERFAZ MÓVIL V12: PUNTOS DE SANEAMIENTO EN LISTA COMPACTA CON EDICIÓN DE UN SOLO PUNTO CADA VEZ
// V1114 - INTERFAZ MÓVIL V11: EL PLANO TÉCNICO DE SANEAMIENTO USA UNA VISTA PROPIA PARA EVITAR EL RECORTE DE DETAILS ANIDADOS EN IPHONE/SAFARI
// V1113 - INTERFAZ MÓVIL V10: LOS BLOQUES ABIERTOS DE SANEAMIENTO CRECEN A SU ALTURA REAL Y EL CONTENEDOR PUEDE DESPLAZARSE HASTA EL FINAL DEL PLANO Y SUS DETALLES
// V1112 - SANEAMIENTO: CARGA PROTEGIDA FRENTE A COPIAS ANTIGUAS DE PROYECTOS; LA NAVEGACIÓN VOLVER/RESUMEN SOLO SE ACTIVA CON UNA VERSIÓN COMPATIBLE
...
// V1107 - INTERFAZ MÓVIL V4: SANEAMIENTO COMPACTA ALTURAS Y ESPACIADOS CONSERVANDO LOS 4 BLOQUES PRINCIPALES
// V1106 - INTERFAZ MÓVIL V3: SANEAMIENTO AGRUPADO EN INSTALACIÓN, CÁLCULO, PLANO Y OBRA, Y NOTAS
// V1105 - INTERFAZ MÓVIL V2: BORRADOR Y RESUMEN EN UNA SOLA FRANJA, PLANO CERRADO AL ENTRAR
// V1104 - INTERFAZ MÓVIL V1: SANEAMIENTO COMPACTO, RESUMEN PRIMARIO Y DETALLES BAJO DEMANDA
// V1103 - PROYECTOS · SANEAMIENTO V12: PLANO TÉCNICO DE RED ADAPTADO A IPHONE SIN RECORTES
// V1102 - PROYECTOS · SANEAMIENTO V12: PLANO TÉCNICO DE RED CON NODOS, RAMAS Y TRAMOS ENLAZADOS
...
const BASE=[
  ...
  },
  {
    ...
    pasos:[
      ...
      "El bloque Obra y recorrido documenta cómo está previsto o cómo se ha comprobado físicamente el saneamiento. Solo se añaden ramales, colectores, bajantes, arquetas, cambios de cota, registros, conexiones/salidas u otros elementos que existan o estén previstos. Las tarjetas se leen en orden de instalación y pueden reordenarse con los controles de subir y bajar.",
      "El botón Asistente de recorrido guía un elemento cada vez. Pregunta los datos uno a uno y reutiliza los que ya estén registrados en el proyecto, por ejemplo longitud, pendiente, diámetro, material o destino del colector cuando estén disponibles. En ramales y otros tramos simples, Desde y Hasta muestran primero puntos de saneamiento, ramales, elementos del recorrido y salidas ya registrados para poder seleccionarlos sin volver a escribirlos. En un colector, el paso de origen cambia a Entradas al colector y permite seleccionar una o varias conexiones; cada entrada conserva su referencia interna y se puede quitar o añadir sin volver a crear el colector. Sigue siendo posible registrar un origen manual cuando todavía no existe como dato del proyecto. Si una tarjeta ya existente tiene datos principales pendientes, muestra Completar con Asistente: ese modo edita la misma tarjeta, conserva conexiones y valores ya registrados y pregunta solo lo que falta, sin crear otro elemento. El técnico puede usar Añadir manualmente cuando prefiera introducir la tarjeta completa directamente.",
      ...
      "Cada elemento puede registrar referencia, ubicación, montaje, conexiones y destino y, según su tipo, longitud, diámetro, pendiente real, material, plantas o niveles, cotas y observaciones. Los campos que no corresponden al tipo elegido se ocultan. En un ramal se mantiene un origen y un destino; en un colector se usa Entradas al colector para registrar uno o varios puntos o tramos que descargan en él, y un único destino aguas abajo. La información antigua que solo tenía Desde se conserva como primera entrada para no perder proyectos existentes. Si un texto ya escrito coincide de forma inequívoca con un dato registrado, Zentryx recupera su referencia interna sin obligar a escribirlo otra vez; el propio elemento queda excluido de las opciones para impedir una referencia a sí mismo.",
      "Cuando uno o varios tramos tienen conexiones suficientes con datos ya registrados, la ficha muestra Esquema técnico de red. Cada punto, ramal, elemento o salida se dibuja como nodo y una referencia compartida aparece una sola vez. Un colector con varias entradas se representa como convergencia: los puntos o tramos llegan al nodo del colector y desde éste continúa un único tramo hacia el destino, por lo que la longitud, diámetro y pendiente del colector no se repiten como si pertenecieran a cada aporte. En iPhone y otras pantallas estrechas, cuando un colector recibe varias entradas, cada origen se dibuja como una rama independiente hacia una convergencia común antes del colector; así una entrada no queda colocada de forma que parezca descargar en otra. En móvil, las etiquetas de los tramos se mantienen separadas de las tarjetas de origen y destino para que no oculten nombres ni medidas. El esquema ajusta el dibujo al ancho disponible y se identifica expresamente como topología no a escala. No se considera plano de ejecución porque no representa posiciones físicas ni recorridos medidos. La lista inferior distingue cada elemento como En esquema o Sin enlazar, porque esa marca se refiere al esquema topológico y no a una planta física. El orden numerado es únicamente orden de registro y no representa una conexión física. Para un colector sin dibujo, Zentryx indica si falta enlazar al menos una entrada o su destino.",
      "Debajo del esquema aparece una Vista de ejecución separada. Mientras no existan posiciones medibles o un plano de referencia suficiente, Zentryx muestra Pendiente de geometría y no dibuja una planta física. La vista resume cuántos elementos ya disponen de conexión, longitud, diámetro, pendiente, montaje y cotas, y mantiene plegada la lista de datos que faltan. Las longitudes, diámetros y pendientes ya registrados se reutilizan y no se vuelven a pedir. En Obra y recorrido, Geometría de ejecución registra primero la fuente de las posiciones: plano de planta o documento de referencia, referencias medibles en obra, No se conoce, Lo introduciré luego o No aplica. Si se elige un plano o documento, aparece un campo para identificar esa base y un control para vincular una imagen o PDF. El archivo elegido se sube al pulsar Guardar, no al seleccionarlo. Tras elegirlo, el bloque debe mostrar que el archivo está seleccionado y pendiente de guardar. Zentryx conserva nombre, tipo, tamaño y ruta del archivo dentro de los datos de Saneamiento y usa el almacenamiento existente de trabajos, sin crear tablas nuevas. Identificar o vincular el archivo no crea posiciones: el recorrido físico solo se dibuja cuando haya datos reales suficientes. Cuando la base vinculada es una imagen y existe un elemento identificado, puede registrarse su primer punto real sobre el plano. La primera prueba coloca únicamente el inicio de C1. La colocación se abre a pantalla completa para trabajar con precisión: se puede ampliar con dos dedos, desplazar el plano cuando está ampliado y ajustar la marca con un toque o arrastrándola. Cancelar cierra el visor sin cambiar la posición anterior; ✓ Listo acepta la posición en el borrador local y Guardar saneamiento sigue siendo el guardado definitivo. La posición se guarda como porcentajes del ancho y alto de la imagen, por lo que no depende del tamaño de pantalla. Ese punto no define por sí solo la dirección ni los 6 m de recorrido del colector. Una vez guardado el inicio, puede usarse Añadir punto al trazado para registrar el siguiente punto físico y repetir el proceso en cada cambio de dirección que haga falta. El visor muestra los puntos anteriores como referencia; el último punto puede ajustarse o quitarse antes de guardar. En cuanto existen dos puntos confirmados de un mismo recorrido, la Vista de ejecución muestra sobre la imagen base el tramo físico parcial ya conocido, marcado expresamente como parcial y no completo. Solo se dibujan los puntos y segmentos confirmados: no se prolonga la línea, no se supone la salida y no se presenta como plano terminado. El desplegable Qué falta se ajusta al estado real: con un trazado ya iniciado pide completar el recorrido, colocar las entradas y la salida que todavía no tengan posición y registrar las cotas pendientes, sin volver a indicar que el colector ya colocado carece de posición. Añadir un punto no lo convierte automáticamente en Arqueta 1 ni en final del recorrido. Si se sustituye la imagen base, las posiciones de la base anterior se descartan para evitar coordenadas falsas.",
      "En la ficha del proyecto se evita mostrar dos dibujos abiertos con la misma red. El Esquema técnico de red y la Vista de ejecución quedan primero; el dibujo CTE de dimensionado se conserva como Esquema CTE y cálculo, plegado por defecto y disponible cuando se necesita consultar el cálculo.",
      ...
      "En móvil, Saneamiento abre con una vista corta: el nombre del proyecto ocupa una cabecera menor, el borrador recuperado se muestra en una franja breve y Puntos/Carga/Colector/Ver plano comparten una sola fila. Ver plano abre en un toque el plano técnico de red y muestra cuántos tramos están enlazados frente al total de elementos registrados. Debajo solo aparecen cuatro bloques principales cerrados: Instalación, Cálculo, Obra y recorrido, y Notas. Dentro de Instalación, Puntos de saneamiento se muestra como una lista compacta: cada fila enseña nombre, aparato, cantidad, diámetro, pendiente y estado, y solo el punto que se toca abre su formulario de edición; al abrir otro, el anterior se cierra. Así los datos siguen disponibles sin convertir varios puntos en un formulario vertical continuo. En Obra y recorrido se aplica el mismo criterio: Recorrido y elementos muestra filas compactas con tipo, conexión, medidas conocidas y número de pendientes; la explicación queda en Ayuda · Cómo funciona y solo un elemento permanece abierto durante la edición. Las filas no repiten el estado cuando no existen más datos y, cuando dos tramos son parecidos, priorizan referencia, ubicación y las marcas No se conoce / después para distinguirlos sin abrir el formulario. En Cálculo, Dimensionado de la red muestra primero Pendiente, Ø adoptado y Material, dejando Longitud, uniones, destino y el criterio CTE en bloques cerrados. Comprobación técnica usa la propia cabecera como estado general y evita repetir una tarjeta de Estado; debajo muestra solo tres controles cortos —red, ventilación y recorrido de obra—, mientras Puntos, UD y el resto del detalle quedan en Ver detalle técnico. En Instalación, Ventilación muestra de entrada solo la solución elegida y el recorrido/conexión; las observaciones quedan en Datos adicionales, la explicación normativa en Ayuda · Criterio HS 5 y la comprobación técnica permanece cerrada hasta pedirla. Dentro de esa comprobación solo se muestran los grupos que corresponden a la solución seleccionada. La cabecera usa un estado breve (Base correcta, Revisar o Pendiente) y, cuando la comprobación base es correcta, el aviso final se reduce a una franja corta que recuerda que la revisión final de obra sigue pendiente. En Datos de instalación solo permanecen visibles Nº de plantas del edificio y Conexión identificada/verificada; Referencia de columna/ventilación y Ø de ventilación existente quedan dentro de Datos opcionales, cerrado por defecto. En Ventilación primaria se mantiene visible el ramal más largo y solo se muestran como datos principales la prolongación a exterior/cubierta, el tipo de cubierta y la altura de salida; Bajante sobredimensionada queda en Dimensionado y las comprobaciones de tomas de aire, huecos, marquesinas y protección se agrupan en Distancias y entorno, ambos cerrados por defecto. En Distancias y entorno, Altura sobre hueco más alto solo se muestra cuando se indica que existe un hueco habitable a menos de 6 m; con No queda oculta.",
      ...
      "En Técnico, si el conducto es Circular se muestra Diámetro; si es Rectangular se muestran Ancho y Alto; y si está Sin dimensionar no se muestran dimensiones. Los valores de la otra forma se conservan al cambiar de selección, pero no intervienen en el cálculo mientras esa forma no esté activa.",
