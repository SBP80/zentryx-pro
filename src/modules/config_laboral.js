...
// V3062 - BASE EMPRESA + FESTIVOS VERIFICADOS + GEO ASISTIDA
// ===============================
(function(){
"use strict";

const ZX_VERSION="3062";
...
function instalarCSS(){
  ...
  .zx_lab_grid2{display:grid;grid-template-columns:1fr;gap:10px}.zx_lab_hint{font-size:13px;color:#64748b;font-weight:800;line-height:1.4;margin:2px 0 10px}.zx_lab_total{display:inline-flex;background:#e0f2fe;color:#075985;border-radius:999px;padding:8px 11px;font-weight:950;margin-bottom:8px}
  ...
  html[data-zx-theme="dark"] .zx_lab_top{background:rgba(15,23,42,.96)}html[data-zx-theme="dark"] .zx_lab_top strong{color:#f8fafc}html[data-zx-theme="dark"] .zx_admin_row,html[data-zx-theme="dark"] .zx_modal_caja{background:#111827;color:#f8fafc}html[data-zx-theme="dark"] .zx_admin_row_top,html[data-zx-theme="dark"] .zx_modal_top h2{color:#f8fafc}
...
window.ZX_configLaboralReal=async function(){
  ...
  app().innerHTML=`<div class="zx_lab_shell">
    ...
    <div class="zx_card"><h3>Convenio y derechos base</h3><div class="zx_lab_hint">El nombre del convenio no cambia cifras por sí solo. Los derechos deben corresponder al convenio realmente aplicable y sus fechas de vigencia.</div>
      ...
      <div class="zx_lab_grid2"><div><label class="zx_label">Vacaciones anuales</label><input id="vacaciones" type="number" step="0.5" class="zx_input" value="${limpiar(c.vacaciones)}"></div><div><label class="zx_label">Cómputo vacaciones</label><select id="vacaciones_tipo" class="zx_input"><option value="naturales" ${c.vacaciones_tipo!=="laborables"?"selected":""}>Días naturales</option><option value="laborables" ${c.vacaciones_tipo==="laborables"?"selected":""}>Días laborables</option></select></div></div>
      <label class="zx_label">Asuntos propios (horas/año)</label><input id="asuntos_horas" type="number" step="0.25" class="zx_input" value="${limpiar(c.asuntos_horas)}">
    </div>
    ...
    <div class="zx_card"><h3>Precios de horas · base empresa</h3><div class="zx_lab_hint">Son importes base. Un trabajador puede tener precios distintos en Usuarios → Laboral; Fichaje conserva el precio que correspondía cuando se creó cada jornada.</div>
      <div class="zx_lab_grid2"><div><label class="zx_label">Hora normal</label><input id="precio_hora" type="number" step="0.01" class="zx_input" value="${limpiar(c.precio_hora)}"></div><div><label class="zx_label">Extra normal</label><input id="precio_extra" type="number" step="0.01" class="zx_input" value="${limpiar(c.precio_extra)}"></div><div><label class="zx_label">Extra nocturna</label><input id="precio_extra_nocturna" type="number" step="0.01" class="zx_input" value="${limpiar(c.precio_extra_nocturna)}"></div><div><label class="zx_label">Extra festiva</label><input id="precio_extra_festiva" type="number" step="0.01" class="zx_input" value="${limpiar(c.precio_extra_festiva)}"></div></div>
